...
class PoolManager:
    ...
    def evaluate_entries(
        ...
        """
        评估所有活跃的观察池条目（纯评估，无副作用）

        术语说明：
            - entries: 指观察池中的多个 PoolEntry（跟踪记录），不是K线数据
            - price_data: 当前K线数据，用于评估价格是否满足买入条件

        评估过程：
            对每个活跃的 PoolEntry，使用其历史信息（突破价、峰值价）与当前K线数据
            进行对比，判断是否满足买入条件。

        无副作用：
            此方法只返回评估结果，不会修改池状态、移除条目或转移条目。
            如需执行动作，请调用 apply_evaluation_results()。

        Args:
            price_data: 当前K线数据字典
                - key: 股票代码 (symbol)
                - value: pd.Series，包含 open, high, low, close, volume
            context: 额外评估上下文（如分时数据、成交量基准等）

        Returns:
            List[Tuple[PoolEntry, EvaluationResult]]:
                - PoolEntry: 被评估的观察池条目（跟踪记录）
                - EvaluationResult: 评估结果，包含 action 和评分
        """
        context = context or {}
        results = []
        ...
        for entry in self.get_all_active():
            ...
            result.metadata['current_price'] = current_price
    ...
    def apply_evaluation_results(
        ...
        术语说明：
            - results: evaluate_entries() 返回的 (PoolEntry, EvaluationResult) 列表
            - action: EvaluationResult 中的决策指令（REMOVE/TRANSFER/BUY/HOLD）
            - signal: 当 action 为买入类型时生成的 BuySignal 业务对象

        执行逻辑：
            根据每个 EvaluationResult 的 action 字段执行相应操作：
            - REMOVE: 调用 _handle_remove()，将 PoolEntry 标记为 failed 并从池中移除
            - TRANSFER: 调用 _handle_transfer_to_daily()，将 PoolEntry 从实时池转移到日K池
            - STRONG_BUY/NORMAL_BUY: 创建 BuySignal 对象并返回
            - HOLD: 不执行任何操作，继续观察

        副作用：
            此方法会修改池状态（移除、转移条目）。如果只想查看评估结果而不执行，
            请只调用 evaluate_entries()。

        Args:
            results: evaluate_entries() 返回的结果列表
                - 每个元素是 (PoolEntry, EvaluationResult) 元组

        Returns:
            List[BuySignal]: 满足买入条件的信号列表
                - 只有 action 为 STRONG_BUY 或 NORMAL_BUY 时才会生成
        """
        signals = []
        ...
        for entry, result in results:
            # 更新 entry 评估状态
            if result.metadata.get('current_price', 0) > 0:
                entry.update_evaluation(
                    result.total_score,
    ...
    def check_buy_signals(
        ...
        这是 evaluate_entries() + apply_evaluation_results() 的组合调用。
        适合不需要中间检查评估结果的场景。

        处理流程：
            1. 遍历所有活跃的 PoolEntry（观察池条目）
            2. 对每个条目，用当前K线数据评估是否满足买入条件
            3. 根据评估结果的 action 执行相应操作
            4. 返回所有买入信号

        副作用警告：
            此方法会修改池状态：
            - REMOVE 动作会将条目从池中移除
            - TRANSFER 动作会将条目转移到日K池
            如需先检查评估结果再决定是否执行，请分别调用
            evaluate_entries() 和 apply_evaluation_results()。
    ...
    def _prepare_evaluation_context(
        ...
        if 'prev_close' not in context:
            context['prev_close'] = entry.breakout_price or entry.highest_peak_price
    ...
    def get_entries_with_breakouts(self) -> List[Tuple[PoolEntry, Optional['Breakout']]]:
        ...
        用于构建评估上下文，需要从 Breakout 对象获取 atr_value 等信息。
    ...
    def get_events(self) -> List[PoolEvent]:
        """
        获取所有已发射的事件（回测分析用）

        注意：当前实现不持久化事件历史。
        如需事件历史，请在初始化时注册一个收集事件的监听器。

        Returns:
            事件列表（当前返回空列表，需要外部收集）
        """
        # 注意：事件在发射后不保存，需要外部监听器收集
        # 此方法提供接口兼容性，实际使用时应注册监听器
        return []
