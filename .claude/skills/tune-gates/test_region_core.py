...
def _write_shards(df, tmp_path, n_shards=3):
    """把合成长表按股票切成若干 parquet 分片(模拟扫描端按股票批次落盘),返回分片路径列表。

    切点落在股票边界上:`prepare_shards` 要求分片之间 symbol 不相交。"""
    paths = []
    syms = df["symbol"].unique()
    bounds = np.linspace(0, len(syms), n_shards + 1).astype(int)
    for i in range(n_shards):
        q = tmp_path / f"part-{i:04d}.parquet"
        df[df["symbol"].isin(syms[bounds[i]:bounds[i + 1]])].to_parquet(q, index=False)
        paths.append(q)
    return paths
...
def test_prepare_shards_shrinks_dtypes_and_rejects_empty(tmp_path):
    """分片路径会把常驻数组压到够用为止(它们是全程唯一的大件);没有分片直接报错。

    states 是 bar 计数(不是 one-hot),买点事件口径下还带符号,所以是有符号 int16。"""
    shards = _write_shards(_synth(seed=4, n_sym=20), tmp_path)
    prep = prepare_shards(shards, COMBO, PREDS, "fold", FOLDS)
    assert prep.flat.dtype == np.int32 and prep.states.dtype == np.int16 and prep.sym_codes.dtype == np.int16
    assert prep.seg.dtype == np.int8 and (prep.seg == 1).all() and prep.n_segments == prep.n_rows_raw
