...
def compute_breakouts_from_dataframe(
    ...
    for info in breakout_infos:
        # 特征计算
        bo = feature_calc.enrich_breakout(df, info, symbol, detector=detector)
        breakouts.append(bo)
...
def _scan_single_stock(args):
    ...
    Args:
        args: (symbol, data_dir, total_window, min_side_bars, min_relative_height,
               ...
               start_date, end_date, feature_calc_config, scorer_config)
            ...
            "breakouts": [
                {
                    ...
                    "price_change_pct": float(bo.price_change_pct)
                    if bo.price_change_pct
                    else None,
                    ...
                    "continuity_days": int(bo.continuity_days)
                    if hasattr(bo, "continuity_days") and bo.continuity_days
                    else None,
