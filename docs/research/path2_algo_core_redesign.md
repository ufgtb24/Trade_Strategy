...
- **执行复现**:`edges=[A→B(0,0), B→C(0,0)]`;`A=[ev(0,0)]`,`B=[ev(0,1), ev(0,3)]`,`C=[ev(3,4)]`。`B→C(0,0)` 要求 `C.start == B.end`;`C` 仅 `start=3`,故需 `B.end=3` 即 `B=ev(0,3)`(索引 1)。但 `A→B(0,0)` 下 earliest `B` 取索引 0 `ev(0,1)`(`B.end=1`),`C` 无解;源-only 回溯仅前进 `ptr[A]`,`A` 耗尽 → `[]`。
- **地面真值**:`{A(0,0), B(0,3), C(3,4)}` 唯一可行。
- **修正后期望(回归锚)**:产出 1 个 `PatternMatch`,`children=(A(0,0), B(0,3), C(3,4))`,`start_idx=0`,`end_idx=4`。

### CRITICAL-2 — 多源只回溯第一个源

`for s in sources: if ...: ptr[s]+=1; break` 只推进拓扑序里第一个仍有备选的源即 `break`;其余源指针永不独立回溯。多源 DAG 丢匹配。

- **执行复现**(已对真实 `advance_dag` 运行,见 CHECK-3 脚本):`edges=[A→C(1,3), B→C(0,0)]`;`A=[ev(0,1), ev(1,3), ev(4,5)]`,`B=[ev(0,1), ev(1,1), ev(5,6)]`,`C=[ev(3,3), ev(3,6), ev(6,7)]`。真实 `advance_dag` 返回 `[]`。
- **地面真值**(穷举 + 精确 gap 公式):2 个可行 — `{A(1,3), B(5,6), C(6,7)}`、`{A(4,5), B(5,6), C(6,7)}`。
...
- **CHECK-3 隔离实验(裁定依据)**:在现行代码上**只**修 CRITICAL-2(穷举所有源索引组合 = 完整多源回溯),**故意保留**"永不回退" scan 指针不动、保持 `lo>hi` 在 scan 之前的忠实顺序 → 两个地面真值匹配全部恢复。结论:该 `[]` **完全由 CRITICAL-2 造成**,与任何 scan 指针携带无关。这同时证明了 mode-ii 假设在现行代码上不成立(裁定见 §0 注)。
- **修正后期望(回归锚)**:产出 2 个 `PatternMatch`,`children` 分别为 `(A(1,3),B(5,6),C(6,7))` 与 `(A(4,5),B(5,6),C(6,7))`,按 `end_idx` 升序 yield(本例两者 `end_idx` 同为 7,顺序任意,符合 spec §1.2.2)。
...
- **执行复现**:任一会产出两个 `(s,e)` 相同匹配的输入(例如 Kof 同窗口选不同成员,或 Dag 两组不同成员张成同一 `[min start, max end]`)。`run(detector)` 在第二个匹配处抛 `ValueError: event_id 单 run 内重复:{label}_{s}_{e}`。
- **修正后期望(回归锚)**:两个匹配均产出,`event_id` 分别为干净的 `{label}_{s}_{e}` 与 `{label}_{s}_{e}#1`,`run()` 不抛错。
...
剪枝:**FAILED 前沿割记忆**(INV-C)。结构事实:一个已选实例只通过其 `end_idx` 影响后继。记忆签名 = 跨越"已赋值→未赋值后缀"的所有边的尾端 `end_idx` 组成的**前沿割**;相同前沿割签名 ⇒ 对所有剩余节点的可行域相同 ⇒ 记忆健全可剪。候选级推论:同一 admissible 集合内**只对不同 `end_idx` 递归**(值相等簇塌缩为 1 次探测)。
...
- **Architecture 行 + Task 5**:把 `advance_dag` 重写为 LEF-DFS + 携带整数下标的非重叠消费(IMPORTANT-1)+ INV-A/B/C 三态分离 + 共享 detect()-局部 `seen_ids`+`#seq`(CRITICAL-3)。把 §1 全部 5 个回归锚 + §8.4 的 C1/A3 锚加入 Task 5 测试。复杂度散文按 §5 改。
