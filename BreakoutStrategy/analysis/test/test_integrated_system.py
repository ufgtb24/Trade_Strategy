...
def test_integrated_system():
    ...
    for info in breakout_infos:
        ...
        bo = feature_calc.enrich_breakout(df, info, "AAPL", detector=detector, atr_series=atr_series)
        breakouts.append(bo)
    ...
    for i, bo in enumerate(sorted_breakouts[:5], 1):
        ...
        print(
            f"    放量: {bo.volume_surge_ratio:.2f}倍, PBM: {bo.momentum*1000:.2f}‰, 稳定性: {bo.stability_score:.0f}%"
        )
        print("    ---")
