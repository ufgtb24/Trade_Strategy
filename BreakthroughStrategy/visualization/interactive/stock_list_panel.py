...
class StockListPanel:
    ...
    def _create_ui(self):
        ...
        self.fixed_tree.column("Symbol", width=100, anchor=tk.W, stretch=False)
    ...
    def load_data(self, scan_results: Dict):
        ...
        for result in scan_results.get("results", []):
            ...
            item["avg_quality"] = avg_quality
            item["max_quality"] = max_quality
        ...
        if self.stock_data:
            ...
            columns = [k for k in first_item.keys() if k not in ["symbol", "raw_data"]]

            # 排序：确保重要字段在前
            priority = ["bts", "active_peaks", "avg_quality", "max_quality"]
            columns.sort(key=lambda x: priority.index(x) if x in priority else 999)

            self._configure_tree_columns(columns)  # 初始化过滤数据
