...
class Breakout:
    ...
    gap_atr_ratio: float       # 跳空的 ATR 标准化值
    volume_surge_ratio: float       # 放量倍数
    momentum: float                 # 突破前涨势强度 (PBM)
    stability_score: float          # 稳定性分数

    # 日间涨幅 ATR 标准化：(close[i] - close[i-1]) / atr[i-1]
    daily_return_atr_ratio: float = 0.0

    # 近期 peak 凹陷深度（0 表示无近期 peak）
    # pk_momentum = 1 + log(1 + D_atr)，其中 D_atr = (peak - trough) / ATR
    pk_momentum: float = 0.0
    ...
    days_since_last_breakout: Optional[int] = None  # 距上次突破的交易日间隔，None=首次突破
