...
> 状态：已实现 (Implemented) | 最后更新：2026-01-08
...
ATR相关特征:
  - daily_return_atr_ratio: 日间涨幅的ATR标准化值 (close[i]-close[i-1])/atr[i-1]
    用于奖励单根强势K线
  - spike_ratio: 累积涨幅的ATR标准化值
    max((close[i]-close[i-var])/atr[i-var]) for var in [1, spike_window]
    用于惩罚近期超涨（spike_penalty）
  - atr_value: ATR值（前一天的ATR）
  - atr_normalized_height: 突破幅度/ATR（可选功能）
...
spike_window: 5                    # 累积涨幅检测窗口（用于 spike_penalty）
...
# 注：QualityScorer 是 BreakoutScorer 的向后兼容别名
import pandas as pd
...
### 2026-01-08 Spike Penalty 重构：累积涨幅检测

**改动**：
- `daily_return_atr_ratio` 简化为 `(close[i] - close[i-1]) / atr[i-1]`，专注于单日强势K线
- 新增 `spike_ratio` 字段：`max((close[i] - close[i-var]) / atr[i-var])` for var in [1, spike_window]
- 新增 `spike_window` 参数（默认5），控制累积涨幅检测窗口
- 移除 `baseline_atr_window` 参数（不再需要滑动窗口最小ATR）
- `spike_penalty` 评分改用 `spike_ratio` 而非 `daily_return_atr_ratio`

**原因**：
- 两个指标目的不同：`daily_return_atr_ratio` 奖励单根强势K线，`spike_penalty` 惩罚近期超涨
- 旧方案中连续两天暴涨时，第一天推高ATR导致第二天的ratio被稀释，spike容易逃逸惩罚
- 新方案使用累积涨幅检测：从任意起点算起涨幅过大都会触发惩罚
- 使用各自起点的ATR（`atr[i-var]`）避免ATR被后续暴涨污染

**性能优化**：
- `spike_ratio` 计算使用 numpy 向量化操作，避免 Python 循环
