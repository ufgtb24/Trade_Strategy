...
class BreakthroughPlotter(BasePlotter):
    ...
    def plot_full_analysis(self,
        ...
        # 7. 添加标题和图例
        if title is None:
            title = f"Breakthrough Analysis ({len(breakthroughs)} breakthroughs detected)"
    ...
    def plot_breakout_detail(self,
        ...
        subtitle = f"{breakthrough.num_peaks_broken} peaks broken | Quality: {breakthrough.quality_score:.1f}" if breakthrough.quality_score else ""
    ...
    def plot_parameter_comparison(self,
        ...
        for i, param_value in enumerate(param_values):
            ...
            if param_value not in param_results:
                ax.text(0.5, 0.5, f'No data for {param_name}={param_value}',
            ...
            # 绘制峰值
            if detector and hasattr(detector, 'active_peaks'):
                self.marker.draw_peaks(ax, df, detector.active_peaks,
                                     quality_color_map=True)

            # 绘制突破
            self.marker.draw_breakthroughs(ax, df, breakthroughs,
                                         highlight_multi_peak=True)
    ...
    def plot_multi_peak_cases(self,
                             df: pd.DataFrame,
        ...
        multi_peak_breakthroughs = sorted(
            ...
            key=lambda bt: bt.num_peaks_broken,
            reverse=True
        )[:top_n]
        ...
        for i, bt in enumerate(multi_peak_breakthroughs):
            ...
            # 绘制标记
            self.marker.draw_peaks(ax, df_subset, adjusted_peaks, quality_color_map=True)
            self.marker.draw_breakthroughs(ax, df_subset, [adjusted_bt], highlight_multi_peak=True)
        ...
        # 添加主标题
        self.add_title(fig, f'Top {len(multi_peak_breakthroughs)} Multi-Peak Breakthroughs')
