...
- 产出按 design §2 锁定形状:单一 `PatternMatch(Event)`,`children` 按 `start_idx` 升序(spec §3.3),`role_index` 标签→该标签命中的唯一 `Event`,`pattern_label`。形状不变。
...
| **Kof** | **是**(复用 §10.5 LEF 生产循环;Part D 与接受谓词无关) | **单 WCC 零 / 多 WCC ≤(p−1)**(与 Dag 同;§10.6) | ~~窗口右界封口~~ **作废**(滑窗+重排模型经 CRITICAL-2 推翻);代价转移至时间(标签维指数,§10.6) |
...
9. **`run()` 驱动不变式**:`end_idx` 升序、`event_id` 单 run 唯一(含 `#seq` 路径)、`role_index`/`children` 一致性(`set(role_index.values()) == set(children)`)。
...
## 10. Kof 语义与算法裁定(2026-05-17 补;tom 裁定,team-lead 接受;**Kof 部分以本节为权威**)

> 背景:LEF-DFS 重设计 team 刻意未打磨 Kof,§3.3/§4/§6 对 Kof 只给草图。opus 对抗审查在 Kof 实现查出 CRITICAL-1(漏匹配)、CRITICAL-2(缓冲无界)、IMPORTANT-2(非重叠 vs 枚举矛盾)。根因 = 文档未充分定义 Kof。本节裁定 Kof 精确语义+算法,**取代 §3.3/§4/§6/§2.3 中关于 Kof 的旧表述**(那些标"滑窗计数/有界 max_gap 视界缓冲/O(ΣN·n)"的字样**作废**)。协议层 `path2/` / spec 仍零改动。

### 10.1 精确语义

出现标签集 `V` = edges 端点并集。在消费前沿后缀上,一次 Kof 命中 = 赋值 `φ: V → Event` 满足:(1) 每标签 `L` 取 `S[L][ptr[L]:]` 中一实例;(2) **k-of-n 谓词**:`|{e∈edges: min_gap(e) ≤ φ(e.later).start − φ(e.earlier).end ≤ max_gap(e)}| ≥ k`;(3) **全标签在场** `dom(φ)=V`(**no partial match**,见 10.4);(4) **earliest-feasible = start-first key** `(start_idx,end_idx,position)`,与 LEF-DFS 同一把 key(§2.2);(5) **全成员非重叠消费** `ptr[L]:=position(φ(L))+1`(§6 Part D 同族)。生产 = 重复"求满足 1–4 的命中→产出→按 5 推进→无命中则推进最早源后重试,直到源后缀耗尽"。

### 10.2 成员选择 = 枚举/回溯(裁定,修 CRITICAL-1)

逐标签盲取 start-first argmin 一个候选不健全(repro:`edges=[A→B(10,10)]`,`A=[ev(0,0)]`,`B=[ev(1,1),ev(10,10)]`,k=1;盲取 B(1,1) gap=1∉[10,10] 漏掉合法 `{A(0,0),B(10,10)}`)。**Kof 成员选择必须枚举/回溯**:这正是 §2.3"枚举仅 Kof"的本意——松弛破坏全满足单调,Chain/Dag 单条 LEF 贪心序列够,Kof 必须在成员组合空间枚举。

### 10.3 非重叠 vs 枚举:两个正交维度(修 IMPORTANT-2 矛盾)

文档把两维混淆。**维度 A 成员选择** = 枚举/回溯(§10.2);**维度 B 产出非重叠** = 是,与 Chain/Dag/Neg 同属 §6 Part D 承重不变式。"枚举仅 Kof" 仅指维度 A;plan 行 1257 "非重叠" 正确、"贪心" 错误(应为"枚举/回溯")。Kof 允许重叠会破坏 §1.2.2 端序单调(喂大缓冲)且命中组合爆炸无表达意义。

### 10.4 role_index 完整性 = 全标签在场,no partial(裁定)

Kof 松弛的是**边 gap 约束满足数**,**绝不松弛标签出现完整性**。某标签后缀无候选 ⇒ 该前缀无 Kof 命中(不产出缺标签的部分命中)。与 `test_kof_2_of_3_satisfied` 断言全标签在场对称;与 §2.3 锁定产出形状自洽。

### 10.5 算法:LEF-DFS 结构姊妹(`_kof_dfs` / `_kof_produce_wcc`,不改 `_lef_dfs`)

Kof 复用 LEF-DFS 的 DFS+回溯+key 序+全成员非重叠消费+WCC 框架+共享 `_emit`/`#seq`。**仅 4 处差异**(故新增独立 `_kof_dfs`,**不**给已定案的 `_lef_dfs` 加 mode 分支,避免污染热路径/已打磨核心):

| 机制 | Dag `_lef_dfs` | Kof `_kof_dfs` |
|---|---|---|
| 节点候选窗口 `[lo,hi]` | 由前驱推出过滤 | **无窗口过滤**(边可不满足,不能据某边裁候选);候选=整后缀 |
| 接受谓词 | 隐式(全节点落窗⇒全满足) | **叶子(k==len(order))显式统计 sat≥k 才接受,否则回溯** |
| INV-C 前沿割记忆 | 健全 | **关闭**(无窗口约束⇒前沿割不表征剩余可行域) |
| C1 等-end 塌缩 | 适用 | **不适用**(gap 看 start 和 end;等-end 不同 start 对 sat 贡献不同,全候选都要试) |
| INV-A 全后缀 scan / INV-B 非重叠消费 / key 序 / WCC 框架 / `_emit` | — | **全部直接复用** |

伪代码(设计级,无仓库代码):

```
function KOF_PRODUCE(G,S,edges,k,label):
    order←TOPO(G); ptr←{L:0}; seen_ids←{}; sources←入度0节点
    loop:
        if any ptr[s]≥len(S[s]) for s in sources: return
        r←KOF_DFS(order,0,{},{},ptr,S,edges,k)        # 无 memo
        if r is None: 推进最早仍有备选的源 or return; continue
        φ,chosen←r
        emit PATTERN_MATCH(φ,label,seen_ids)          # 复用 _emit,#seq 字节一致
        for L in φ: ptr[L]←chosen[L]+1                # INV-B 全成员非重叠

function KOF_DFS(order,j,assign,chosen,ptr,S,edges,k):
    if j==len(order):
        sat←Σ_{e∈edges}[ min_gap(e) ≤ assign[e.later].start − assign[e.earlier].end ≤ max_gap(e) ]
        return (dict(assign),dict(chosen)) if sat≥k else None
    v←order[j]
    cands←[(S[v][i],i) for i in range(ptr[v],len(S[v]))]   # 无窗口过滤
    cands←SORT_BY_KEY(cands)                                # (start,end,position)
    for (e,i) in cands:                                     # 不做等-end 塌缩
        assign[v]←e; chosen[v]←i
        r←KOF_DFS(order,j+1,assign,chosen,ptr,S,edges,k)
        if r≠None: return r
        del assign[v]; del chosen[v]
    return None
```
...
### 10.7 新增隐患裁定:Kof 必须单 WCC(`validate_kof` 加校验)

跨 WCC 的 k-of-n 与逐 WCC 分解(缓冲/产出顺序所需)冲突:跨 WCC 边在逐 WCC DFS 无法统计。但共享端点的标签必弱连通(同 WCC);**多 WCC 的 k-of-n 无明确表达意义**。裁定:**`validate_kof` 新增"edges 必须弱连通(单 WCC)"校验**(类比 `validate_chain` 的 `_connected`),不连通 → 构造期 `ValueError`。这同时关闭隐患、保零缓冲、使 k-of-n 全局语义自洽。**这是 Kof 裁定引入的唯一新增 stdlib 校验改动;协议层/spec 仍零改动。** k=n 时语义=全满足(可选实现优化:退回 `advance_dag`;**仅注记,不强制**,保持 Kof 单一实现清晰)。

### 10.8 回归锚(钉死,重实现 TDD)

- **A-KOF-C1**(修 CRITICAL-1):`edges=[TemporalEdge("A","B",10,10)]`,k=1,`A=[ev(0,0)]`,`B=[ev(1,1),ev(10,10)]` → 恰 1 命中,role_index keys={"A","B"},A=ev(0,0) B=ev(10,10),children=(A(0,0),B(10,10)),start=0 end=10(旧实现 `[]`)。
- **A-KOF-C2**(修 CRITICAL-2,零缓冲):`edges=[TemporalEdge("A","B",0,inf)]`,k=1,`A=[ev(i,i) i∈0..N-1]`,`B=[ev(i,1_000_000_000+i)]`,N=3000 → 产出 N 个,emitted end_idx 单调不减,任意时刻未 yield 缓冲 ≤ 常数(单 WCC 零),`run(d)` 不抛不 OOM(旧实现缓冲 O(N))。
- **A-KOF-OV**(修 IMPORTANT-2 非重叠):`edges=[TemporalEdge("A","B",0,5)]`,k=1,`A=[ev(0,0),ev(1,1)]`,`B=[ev(2,2)]` → 恰 **1** 命中(B(2,2) 被首命中消费后耗尽)(旧实现 2 个重叠)。
- **A-KOF-NP**(no partial):`edges=[A→B(0,5),B→C(0,5),A→C(0,2)]`,k=1,`A=[ev(0,0)]`,`B=[ev(1,1)]`,`C=[]` → `list(run(d))==[]`;`C=[ev(3,3)]` → 恰 1 命中,`set(role_index)=={"A","B","C"}`。
- **A-KOF-WCC**(单 WCC 校验):`Kof(edges=[TemporalEdge("A","B"),TemporalEdge("C","D")], k=1, A=..,B=..,C=..,D=..)` 构造期 `ValueError`(不连通)。
- **A-KOF-ID**(共享 #seq 字节一致):`edges=[A→B(0,0)]`,k=1,`A=[ev(5,5),ev(5,5)]`,`B=[ev(5,5),ev(5,5)]` → ids ⊆ {"kof_5_5","kof_5_5#1"},`run()` 不抛;`end_idx` 升序;`set(role_index.values())==set(children)`。
...
- **R6** N 不进:`edges=[TE("A","B",0)]`,`forbid=[TE("A","N",100,200)]`,`A=[ev(0)]`,`B=[ev(0)]`,`N=[ev(0)]` → 1 match,`set(role_index.values())==set(children)`,N 不在任何成员/keys。
