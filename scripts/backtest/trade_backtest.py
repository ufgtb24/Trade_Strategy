...
使用方式：
    python scripts/backtest/trade_backtest.py \
...
from BreakoutStrategy.analysis import BreakoutDetector, Breakout
...
class SimpleBacktestEngine:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.detectors: Dict[str, BreakoutDetector] = {}  # {symbol: detector}

        # 待买入队列（次日执行）
        self.pending_buys: List[Tuple[str, Breakout]] = []
    ...
    def _initialize_detectors(self):
        ...
        for symbol in self.price_data.keys():
            self.detectors[symbol] = BreakoutDetector(
                symbol=symbol,
    ...
    def _process_day(self, current_date: date, all_trading_days: List[date]):
        ...
        self._scan_and_queue_breakouts(current_date)

    def _scan_and_queue_breakouts(self, current_date: date):
        """扫描当日突破并加入待买入队列"""
        ...
        for symbol, df in self.price_data.items():
            ...
            atr_series = TechnicalIndicators.calculate_atr(
                df_until_today["high"], df_until_today["low"], df_until_today["close"], 14
            )
            breakout_infos = [breakout_info] if breakout_info else []
            for info in breakout_infos:
                bo = feature_calc.enrich_breakout(df_until_today, info, symbol, detector=detector, atr_series=atr_series)
                today_breakouts.append((symbol, bo))

        # 批量评分
        if today_breakouts:
            all_bos = [bo for _, bo in today_breakouts]
            scorer.score_breakouts_batch(all_bos)

            # 过滤低质量突破，并按分数排序
            qualified = [
                (symbol, bo) for symbol, bo in today_breakouts
                if bo.quality_score is not None and bo.quality_score >= self.min_quality_score
            ]
    ...
    def _execute_pending_buys(self, current_date: date):
        ...
        executed = []
        for symbol, bo in self.pending_buys:
            # 检查持仓上限
            ...
            # 执行买入
            success = self._execute_buy(symbol, current_date, open_price, bo)
            if success:
                executed.append((symbol, bo))

        # 清空待买入队列
        self.pending_buys.clear()

    def _execute_buy(self, symbol: str, buy_date: date, price: float, bo: Breakout) -> bool:
        """
        执行买入
