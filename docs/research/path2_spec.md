...
- 6 个关系算子(`Before` / `At` / `After` / `Over` / `Any` / `Overlaps`)
- 1 个组合子(`Pattern.all`)
- `TemporalEdge` 数据类
...
- **`predicate` 可选**:
  ...
  - `predicate(idx: int) -> bool`:对 bar 索引求值(`stream=None` 形态,需在 Detector / Pattern 上下文有 `df` 可访问)
...
- **v0.1**(2026-05-12):初稿。基于 2026-05 路径选型期的累计共识固化(对比/倡导底稿已于框架建成后清理;教学见 `docs/path2/path2_tutorial.md`)
