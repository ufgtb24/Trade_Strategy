...
支持双配置文件架构：
- 默认配置文件：configs/scan_config.yaml（只读，作为默认值）
- 用户配置文件：configs/user_scan_config.yaml（用户自定义，可覆盖）
"""
...
class UIScanConfigLoader:
    ...
    采用双配置文件架构：
    - 默认配置文件：configs/scan_config.yaml（只读）
    - 用户配置文件：configs/user_scan_config.yaml（可覆盖）
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        Args:
            config_path: 默认配置文件路径（可选，默认 configs/scan_config.yaml）
        """
        if self._config is not None:
            return  # 已初始化，跳过

        if config_path is None:
            # 默认路径：项目根目录/configs/scan_config.yaml
            current_file = Path(__file__)
            # 当前文件: .../Trade_Strategy/BreakoutStrategy/UI/config/scan_config_loader.py
            # 项目根目录: .../Trade_Strategy
            self._project_root = current_file.parent.parent.parent.parent
            self._default_config_path = (
                self._project_root / "configs" / "scan_config.yaml"
            )
        else:
            self._default_config_path = Path(config_path)
            # 项目根目录：假设配置文件在 configs/ 下
            self._project_root = self._default_config_path.parent.parent

        # 用户配置文件：与默认配置同级
        self._user_config_path = (
            self._default_config_path.parent / "user_scan_config.yaml"
        )
    ...
    def get_filter_config(
        ...
        Returns:
            (min_price, max_price, min_volume) 元组，None表示不限制
        """
        data = self._config.get("data", {})
        return (
            data.get("min_price"),
    ...
    def set_filter_config(
        ...
        if "data" not in self._config:
            self._config["data"] = {}
        self._config["data"]["min_price"] = min_price
        self._config["data"]["max_price"] = max_price
        self._config["data"]["min_volume"] = min_volume
        self._notify_listeners()
    ...
    def get_params_file(self) -> str:
        ...
        params_file = self._config.get("params", {}).get(
            "config_file", "configs/params/all_factor.yaml"
        )
    ...
    def get_label_max_days(self) -> int:
        """
        获取标签配置中的最大天数（用于计算后置缓冲）

        Returns:
            label_configs 中最大的 max_days 值，默认 20
        """
        configs = self.get_label_configs()
        if not configs:
            return 20
        return max(cfg.get("max_days", 20) for cfg in configs)
    ...
    def get_label_config_string(self) -> str:
        ...
        Returns:
            格式化字符串，如 "40"（只显示 max_days）
        """
        configs = self.get_label_configs()
        if configs:
            first = configs[0]
            return str(first.get("max_days", 20))
        return "20"
    ...
    def set_label_config_from_string(self, label_str: str):
        ...
        """
        try:
            max_days = int(label_str.strip())
        except ValueError:
            raise ValueError(f"Invalid number: '{label_str}'")

        if max_days <= 0:
            raise ValueError("Days must be positive")

        self.set_label_configs([{"max_days": max_days}])
        ...
        if "date" not in df.columns or "name" not in df.columns:
            ...
            try:
                ...
                if df.shape[1] >= 2:
                    ...
                    raise ValueError(
                        f"CSV must have at least 2 columns (date, name). "
                        f"Found {df.shape[1]} column(s)."
                    )
            except ValueError:
                raise
            except Exception as e:
                raise ValueError(f"Failed to re-read CSV without header: {e}")
