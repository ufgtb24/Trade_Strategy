...
def adapt_peaks(raw_peaks: list) -> tuple[list[ChartPeak], list[ChartPeak], dict]:
    """把 raw_peaks dict 列表按状态分组成 ChartPeak 列表 + id→ChartPeak 映射。

    返回 (active_peaks, superseded_peaks, by_id)。
    ...
    for p in raw_peaks:
        ...
        all_peaks.append(chart_peak)
        is_superseded = p.get("is_superseded", False)
        if is_superseded:
            superseded_peaks.append(chart_peak)
        else:
            active_peaks.append(chart_peak)

    by_id = {p.id: p for p in all_peaks if p.id is not None}
    return active_peaks, superseded_peaks, by_id
