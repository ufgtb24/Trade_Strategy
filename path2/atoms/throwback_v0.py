...
tb event [start_idx, end_idx] = 突破后可执行的整理买窗:
  ...
  - outcome ∈ ('rise', 'break', 'timeout')= 窗口关闭方式(见 ThrowbackEventV0);
...
anchor = evaluate_throwback 显式入参(None 时回退 measure_at(bo-1, anchor_measure));
detect 消费 burst 流、锚 burst 的 last_bo:anchor = measure_at(last_bo.end_idx - 1,
anchor_measure)(last_bo 上一根 bar 价,非串内 min);Phase 1 全程
measure_at(i, support_measure) ≥ anchor;ATR 取 bo-1(避开 bo 当根异常 TR)。
...
def _find_confirm_idx(df: pd.DataFrame, bo_idx: int, anchor: float,
                    ...
                    scb_measure: str = "low") -> Optional[tuple[int, int]]:
    ...
    confirm 条件(K = stop_confirm_bars):
      - scb_mode="no_new_low"(默认):i - trough_idx ≥ K,且 [trough_idx, i] 内
        至少一根 K 线含 stop signal(_STOP_SIGNALS = 下影/阳线/收涨之一);
      - scb_mode="rising":rising_count ≥ K(rising_count = 从 trough 当根起,
        每根 measure_at(i, scb_measure) ≥ 前根则 +1、下降清零),且
        [trough_idx, i] 内含 stop signal。
      trough_idx = argmin(scb_measure) over [bo+1, i](动态更新;默认 "low" 即
      argmin low,与旧口径一致);当 i 刷新 trough 时,no_new_low 下 i-trough=0、
      rising 下计数归零,天然不满足 K,当前根不 confirm。
    ...
    trough_idx = bo_idx + 1
    rising_count = 0   # scb_mode="rising":trough 当根 0,每根不降 +1,下降清零
    ...
    for i in range(bo_idx + 1, end + 1):
        ...
        if measured_support < anchor:
            ...
            return None
        # 2. maintain trough_idx (argmin over [bo+1, i],统一 scb_measure 口径)
        m_i = measure_at(df, i, scb_measure)
        if m_i < measure_at(df, trough_idx, scb_measure):
            trough_idx = i
            rising_count = 0   # 刷新 trough:新起点
        elif scb_mode == "rising":
            if m_i >= measure_at(df, i - 1, scb_measure):
                rising_count += 1
            else:
                rising_count = 0
...
def _find_end_idx(df: pd.DataFrame, confirm_idx: int, trough_idx: int,
    ...
    for i in range(confirm_idx + 1, end_scan + 1):
        ...
        if lo_i < base_min:
            base_min = lo_i
    debug_break(end_scan, anchor_kind='end')
...
def evaluate_throwback(
    ...
    anchor:显式传入(如 burst 串内最低 bo 价)时用它;None 时回退
    measure_at(bo-1, anchor_measure)(兼容无 burst 上下文调用,旧契约)。
    ATR 取 bo-1(bo 当根异常 TR 污染)。
    atr:预算的 ATR 序列(detect 每股算一次下传);None 时自算。
    Phase 1:确认点 = 首个 i∈[bo+1, bo+max_start_gap] 满足 SCB 条件
      (scb_mode="no_new_low" 时 i-trough≥stop_confirm_bars;"rising" 时连续不降
      计数 ≥stop_confirm_bars,trough/比较均用 scb_measure 口径)
    ...
    if atr <= 0.0:
        return None
    if anchor is None:
        anchor = measure_at(df, bo_idx - 1, anchor_measure)   # 兼容无 burst 上下文调用(旧契约)
    r = _find_confirm_idx(df, bo_idx, anchor, max_start_gap, atr,
                        ...
                        scb_mode=scb_mode, scb_measure=scb_measure)
    if r is None:
        return None
...
class ThrowbackEventV0(Event):
    ...
    输出字段(where 可引用):
    - anchor_bo_id: 本实例来源 bo 的标识(交错标注后取源 bo 的 instance_id(detect 期 bo 已标注);同窗口多 bo 各带单来源);
...
class ThrowbackDetectorV0:
    """派生 detector:消费 burst 流,逐 burst 调 evaluate_throwback,产事件(含失败结局)。

    anchor = last_bo 上一根 bar 的 anchor_measure 价(锚 burst 的 last_bo,非串内 min);
    evaluate 的 bo 参数 = burst 的 last_bo(触发窗预算以最后突破那根为起点)。
    核心判据(详见 evaluate_throwback / _find_confirm_idx / _find_end_idx):
      ATR 取 bo-1 处(避开 BO 当根异常 TR)。
      Phase 1(_find_confirm_idx,找 confirm):扫 [bo+1, bo+max_start_gap];三条不产事件:
        ...
        confirm 条件:i-trough ≥ stop_confirm_bars(默认 scb_mode="no_new_low";
        可选 "rising" = 连续不降计数 ≥ stop_confirm_bars,trough 与比较均按
        scb_measure 口径,默认 "low")且 [trough,i] 内含 stop signal
    ...
    end_idx 升序排序(过 run() 升序不变式):trigger 随 burst 顺序可能乱序,收集后排序再 yield。
    实例流语义:同窗口多 bo 不再合并——每个 bo 各产一条事件(同 span 多实例,由物化
    标注按流序编号,各带单来源 anchor_bo_id);同 span 不同 outcome 属逻辑不可能
    (evaluate 对同 bo 决定性),不额外处理。

    输出字段详见 ThrowbackEventV0。
    """
    has_debug_hooks: ClassVar[bool] = True

    event_cls = ThrowbackEventV0
    ...
    def __init__(self, *, max_start_gap: int = 7, max_window: int = 5,
                 ...
                 scb_mode: str = "no_new_low", scb_measure: str = "low"):
        ...
        if scb_measure not in VALID_MEASURES:
            raise ValueError(f"scb_measure 必须在 {VALID_MEASURES},实际 {scb_measure!r}")
    ...
    def detect(self, burst_stream: Iterable[BurstEvent], df: pd.DataFrame) -> Iterator[ThrowbackEventV0]:
        events = []
        atr_series = calculate_atr(df['high'], df['low'], df['close'], self._kw['atr_window'])
        for burst in burst_stream:
            last_bo = burst.members[-1]
            # anchor = last_bo 上一根 bar 的 anchor_measure 价(锚 last_bo,非串内 min)
            anchor = measure_at(df, last_bo.end_idx - 1, self._kw['anchor_measure'])
            r = evaluate_throwback(last_bo, df, anchor=anchor, atr=atr_series,
                                   on_gate=self.on_gate, **self._kw)
            if r is not None:
                start = r.start_idx
                src_id = last_bo.instance_id
                events.append(ThrowbackEventV0(
