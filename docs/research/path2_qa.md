...
**提问背景**:看了 `docs/path2/path2_by_example.md` 例子 2(BO 后放量),发现 Cond 一行 `VolSpike(conds=[{'ind': Breakout(), 'exp': 5}])` 搞定,Path 2 要先建 2 个 Event 类 + 2 个 Detector + 1 个判定函数。
