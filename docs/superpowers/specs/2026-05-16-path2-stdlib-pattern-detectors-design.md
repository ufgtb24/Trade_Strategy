...
| **Neg** | 正向子图(Chain 或 Dag 形态)+ ≥1 条否定 edge | 正向子图按 Dag 校验;无否定 edge → 报错;**每条 forbid 边须恰一端 ∈ 正向子图节点(正向锚)、另一端 ∉(否定标签)——成员资格 XOR,与 earlier/later 方向无关;两端皆 ∈ 或皆 ∉ → 报错**(redesign §11,取代旧"later 不在正向子图→报错"的单方向错误规则) |
