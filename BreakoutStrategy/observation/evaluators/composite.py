...
class CompositeBuyEvaluator:
    ...
    def evaluate(
        ...
        # ===== 阶段0：准备上下文数据 =====
        # 传递 ATR 值到各维度评估器（用于 ATR 标准化模式）
        if 'atr_value' not in context:
            if hasattr(entry, 'breakout') and entry.breakout:
                atr_value = getattr(entry.breakout, 'atr_value', None)
    ...
    def _create_result(
        ...
        reference_price = entry.breakout_price or entry.highest_peak_price
