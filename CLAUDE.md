...
- **当我说「白话解释 / 通俗解释」**：默认不打比方，用语言化方式把机制本身讲清楚。**一个词该不该解释、能不能用，判据是「它从哪来」，不是「我懂不懂」**——后者只能猜，前者当场查得到。三类词三种待遇：
  - **`CONTEXT.md` 定过的词 → 照用，并带上 `path2/CONTEXT.md:行号`。** 这类词**不在回避之列，反而要优先用**——「引用槽」「复合事件」「事件流」「物化」这些词条本身就是白话，比自造的替代说法更准；换成自造词（「对象引用」「容器」「贴身份」）是在**制造**术语而不是消除术语，实测会直接让解释读不懂。**但出处不是可选项**：词表是 `/grill-with-docs` 当场谈定就写进去的，敲定那一刻我懂、几周不用就忘，之后天天用它的是你不是我。grep 词表时顺手把行号带回来，终端里我点一下就能跳。
  - **某个 skill 的内部词 → 照那个 skill 的人话译法表翻译，一个都别漏出来。** 要给我解释某个 skill 管的事情时**先查它有没有这张表**——`.claude/skills/tune-gates/SKILL.md` 第三节「禁止词与人话译法」是范本（工作点 → 「现在这套参数」，确认窗 → 「留着没碰过的数据」，两段说「往前那段」「往后那段」）。那张表写的是 skill 运行期输出，**在 skill 之外讲同一件事时同样照它说**（写 `docs/explain/` 的文档、日常聊天里解释结论都算）。在词表里加个注解不能抵消：正文每出现一次，我就得回头查一次。
  - **项目外引进来的概念（统计方法、文献术语）→ 不出现它的名字，只讲它在这件事里起什么作用**：τ² 说成「不同时段效果本来就会差多少」，可信度收缩说成「按样本多少打个折」。这一类我是零基础、又没有出处可链，**只有这一类允许打比方**。
  - 没定义过的行话、函数名 / 行号 / 内部字段名一律不出现；回避它们的办法是换成上面三类里对应的说法，**不是换成自己临时造的词**。
  - **我说「完全不懂 / 我是小白」时**，放宽的是上一类的比喻尺度，不是把 `CONTEXT.md` 的词也换成自造说法——我缺的是词条的即时回忆，不是理解力；按「小白」稀释密度，只会让我更难从文档里做决定。
