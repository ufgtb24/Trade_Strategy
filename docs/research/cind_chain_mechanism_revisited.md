...
`EventChain` 是 backtrader-independent 的 Condition_Ind 等价物。每个 `EventChain` 维护**两份逻辑**:

```python
class EventChain:
    """链式机制的核心抽象 — 替代 Condition_Ind 但完全不依赖 backtrader

    每个 EventChain 维护两份逻辑:
    ...
    def evaluate_at(self, df, idx, mode: Literal['training', 'live']) -> Optional[float]:
        """统一接口,根据 mode 判断 lookforward 是否可用"""
        ...
```

**双模式同源**的理由:mining 需要 batch(性能 + 直接喂模板挖掘),live 需要 stream(实时进场)。语义只写一份,驱动器分叉。
...
### Stage 2.3(4-5 周)— Stream 模式 + 新事件类型

**触发条件**(全部满足):
- live 端实际遇到"非 BO 事件"的进场需求
- Stage 2.2 的 batch 模式在 live 端因重复评估开销显著(实测 > 50ms / bar)
- Stage 2.2 中至少 3 个 EventChain 已在生产稳定运行

具体:
1. `EventChain.update_stream(bar)` 接口 + 内部 buffer
2. `TemporalEventDetector`(从 chain 输出事件 row)
3. `EventDetector` 抽象,`BreakoutDetector` 退后到此基类下
4. mining pipeline 抽象为 `(EventDetector, FactorRegistry, LabelFn) -> trial_yaml`
5. **首个 PlatformDetector 上线**
