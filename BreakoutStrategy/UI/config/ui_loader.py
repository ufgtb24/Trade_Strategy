...
class UIConfigLoader:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            # 默认路径：项目根目录/configs/signals/ui_config.yaml
            # 从当前文件向上查找项目根目录
            current_file = Path(__file__)
            # 当前文件: .../Trade_Strategy/BreakoutStrategy/UI/config/ui_loader.py
            # 项目根目录: .../Trade_Strategy
            project_root = current_file.parent.parent.parent.parent
            config_path = project_root / "configs" / "signals" / "ui_config.yaml"
            # 设置项目根目录
            self._project_root = project_root
        else:
            config_path = Path(config_path)
            # 项目根目录：从配置文件位置向上查找
            # config_path: .../Trade_Strategy/configs/signals/ui_config.yaml
    ...
    def save_config(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            config_path = (
                self._project_root / "configs" / "signals" / "ui_config.yaml"
            )
        else:
            config_path = Path(config_path)
    ...
    def get_time_range_for_stock(
        ...
        优先级：
        1. JSON 中的 per-stock 时间范围 (scan_start_date, scan_end_date)
        2. JSON metadata 中的全局时间范围 (start_date, end_date)
        3. 返回 (None, None)
        ...
        # 2. Fallback: 使用 JSON metadata 中的全局时间范围
        # （兼容旧版 JSON 中无突破股票缺少 scan_start_date/scan_end_date 的情况）
        metadata = json_data.get("scan_metadata", {})
        global_start = metadata.get("start_date")
        global_end = metadata.get("end_date")
        if global_start and global_end:
            return (global_start, global_end)

        return (None, None)

    def get_label_configs_from_json(
        self, json_data: Optional[Dict] = None
    ) -> Optional[List[Dict[str, int]]]:
        """
        从 JSON metadata 提取 label_configs

        Args:
            json_data: 扫描结果 JSON 数据

        Returns:
            label_configs 列表，如 [{"min_days": 5, "max_days": 20}]
            如果未找到，返回 None
        """
        if not json_data:
            return None

        feature_params = json_data.get("scan_metadata", {}).get(
            "feature_calculator_params", {}
        )
        label_configs = feature_params.get("label_configs")

        return label_configs if label_configs else None

    def get_label_max_days_from_json(
        self, json_data: Optional[Dict] = None
    ) -> Optional[int]:
        """
        从 JSON metadata 提取 label_max_days（label_configs 中的最大 max_days）
        ...
        Returns:
            ...
            如果未找到，返回 None
        """
        label_configs = self.get_label_configs_from_json(json_data)
        if not label_configs:
            return None

        return max(cfg.get("max_days", 20) for cfg in label_configs)
    ...
    def get_display_options_defaults(self) -> Dict[str, bool]:
        ...
        return self._config.get("ui", {}).get(
            "display_options", {"show_bo_score": True}
        )
    ...
    def get_scan_date(self) -> Optional[str]:
        ...
        Returns:
            日期字符串（YYYY-MM-DD 格式），如果未设置或为 auto-infer 则返回 None
        """
        return self._config.get("scan_settings", {}).get("scan_date")

    def set_scan_date(self, date_str: Optional[str]):
        """
        保存 scan_date 设置到配置

        Args:
            date_str: 日期字符串（YYYY-MM-DD 格式）或 None（表示 auto-infer）
        """
        if "scan_settings" not in self._config:
            self._config["scan_settings"] = {}
        self._config["scan_settings"]["scan_date"] = date_str
        self.save_config()

    def get_chart_lookback_days(self) -> Optional[int]:
        """
        获取图表显示的回溯天数

        Returns:
            回溯天数，如果未配置则返回 None（显示全部数据）
        """
        return self._config.get("ui", {}).get("chart_lookback_days", 252)
    ...
    def get_stock_list_column_config(self) -> Dict[str, Any]:
        ...
        default = {
            ...
            "visible_columns": ["total_breakouts", "active_peaks", "max_quality"],
            "column_priority": ["total_breakouts", "active_peaks", "avg_quality", "max_quality"],
            "column_labels": {},
        }
    ...
    def get_display_config(self) -> Dict[str, int]:
        """
        获取显示配置（from display_config.yaml）

        Returns:
            包含 before_months 和 after_months 的字典
        """
        config_path = self._project_root / "configs" / "signals" / "display_config.yaml"
        default = {"before_months": 6, "after_months": 1}
        ...
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                ...
                return config.get("display", default)
        except Exception:
            return default

    def set_display_config(self, before_months: int, after_months: int):
        """
        保存显示配置到 display_config.yaml

        Args:
            before_months: lookback 前显示的月数
            after_months: lookback 后显示的月数
        """
        config_path = self._project_root / "configs" / "signals" / "display_config.yaml"
        config = {
            "display": {
        ...
        with open(config_path, "w", encoding="utf-8") as f:
            ...
            f.write("# 独立的显示配置，仅控制K线图表显示范围，不参与信号检测\n\n")
            yaml.dump(config, f, allow_unicode=True, default_flow_style=False)


    def get_symbol_filter_config(self) -> Dict[str, bool]:
        """
        获取符号筛选配置

        Returns:
            符号筛选配置字典，如 {"show_bo": True, "show_hv": True, ...}
        """
        default = {
            "show_bo": True,
            "show_hv": True,
        ...
        return self._config.get("ui", {}).get("symbol_filter", default)

    def set_symbol_filter_config(self, config: Dict[str, bool]):
        """
        保存符号筛选配置
        ...
        if "ui" not in self._config:
            self._config["ui"] = {}
        self._config["ui"]["symbol_filter"] = config
        self.save_config()

    def get_forward_return_config(self) -> Dict[str, Any]:
        """
        获取前瞻涨幅配置
        ...
        Returns:
            ...
            - days: int - 前瞻天数
        """
        default = {"enabled": True, "days": 42}
        return self._config.get("scan_settings", {}).get("forward_return", default)

    def set_forward_return_config(self, config: Dict[str, Any]):
        """
        保存前瞻涨幅配置
        ...
        if "scan_settings" not in self._config:
            self._config["scan_settings"] = {}
        self._config["scan_settings"]["forward_return"] = config
        self.save_config()
    ...
    def get_stock_filter_config(self) -> Dict[str, Any]:
        ...
        return self._config.get("scan_settings", {}).get("stock_filter", default)

    def set_stock_filter_config(self, config: Dict[str, Any]):
        """
        保存股票过滤配置
        ...
        if "scan_settings" not in self._config:
            self._config["scan_settings"] = {}
        self._config["scan_settings"]["stock_filter"] = config
        self.save_config()
