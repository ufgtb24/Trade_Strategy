...
class PriceConfirmConfig:
    ...
    # === ATR 标准化配置 ===
    # 是否启用 ATR 标准化（启用后百分比阈值被 ATR 阈值覆盖）
    use_atr_normalization: bool = False

    # 确认区间（ATR 倍数）
    confirm_zone_min_atr: float = 0.3   # 最小确认偏离
    confirm_zone_max_atr: float = 0.8   # 最大确认偏离（超过开始扣分）

    # 下跌阈值（ATR 倍数，带负号）
    pullback_threshold_atr: float = -0.5   # 回踩警戒线
    remove_threshold_atr: float = -1.0     # 跌破移出线

    # 追高阈值（ATR 倍数）
    max_chase_atr: float = 1.5             # 追高上限

    # Bonus 配置
    ideal_zone_bonus: float = 1.2          # 理想区间 bonus
    marginal_zone_penalty: float = 0.8     # 边缘区间 penalty
...
class BuyConditionConfig:
    ...
    def from_dict(cls, data: dict) -> 'BuyConditionConfig':
        ...
        if 'price_confirm' in data:
            ...
            # ATR 标准化配置
            if 'use_atr_normalization' in pc_data:
                config.price_confirm.use_atr_normalization = pc_data['use_atr_normalization']
            if 'confirm_zone_min_atr' in pc_data:
                config.price_confirm.confirm_zone_min_atr = pc_data['confirm_zone_min_atr']
            if 'confirm_zone_max_atr' in pc_data:
                config.price_confirm.confirm_zone_max_atr = pc_data['confirm_zone_max_atr']
            if 'pullback_threshold_atr' in pc_data:
                config.price_confirm.pullback_threshold_atr = pc_data['pullback_threshold_atr']
            if 'remove_threshold_atr' in pc_data:
                config.price_confirm.remove_threshold_atr = pc_data['remove_threshold_atr']
            if 'max_chase_atr' in pc_data:
                config.price_confirm.max_chase_atr = pc_data['max_chase_atr']
            if 'ideal_zone_bonus' in pc_data:
                config.price_confirm.ideal_zone_bonus = pc_data['ideal_zone_bonus']
            if 'marginal_zone_penalty' in pc_data:
                config.price_confirm.marginal_zone_penalty = pc_data['marginal_zone_penalty']
