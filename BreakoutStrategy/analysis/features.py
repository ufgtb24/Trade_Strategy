...
class FeatureCalculator:
    ...
    def enrich_breakout(
        ...
        if annual_volatility > 0 and n_bars > 0:
            ...
            momentum = raw_momentum * math.sqrt(n_bars) / daily_vol
        else:
            momentum = 0.0
        ...
        # 计算距上次突破的交易日间隔（用于 drought_bonus）
        days_since_last_breakout = None
        if detector is not None:
            days_since_last_breakout = detector.get_days_since_last_breakout(idx)
        ...
        return Breakout(
            ...
            days_since_last_breakout=days_since_last_breakout,
