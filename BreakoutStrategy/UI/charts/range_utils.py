...
class ChartRangeSpec:
    ...
    # 降级语义 = pkl 数据是否真的覆盖不到请求范围。直接比较 pkl 边界 vs ideal
    # 日期；不用 *_actual（它们是 pkl 内第一个/最后一个交易日）避免当 ideal
    # 落在周末/节假日时 actual 被抵近到相邻交易日而产生误报。
    @property
    def scan_start_degraded(self) -> bool:
        return self.pkl_start > self.scan_start_ideal

    @property
    def scan_end_degraded(self) -> bool:
        return self.pkl_end < self.scan_end_ideal

    @property
    def compute_buffer_degraded(self) -> bool:
        return self.pkl_start > self.compute_start_ideal
