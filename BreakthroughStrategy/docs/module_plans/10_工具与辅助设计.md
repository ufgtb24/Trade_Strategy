...
工具与辅助模块提供系统运行所需的基础工具和辅助功能，包括日志记录、数据库管理、数据可视化和日期时间处理等通用功能。

**核心职责**：
1. 提供统一的日志系统（多级别、多输出、日志轮转）
2. 管理数据库连接和操作（SQLite/PostgreSQL）
3. 提供数据可视化工具（K线图、指标图表、回测报告）
4. 提供日期时间工具（交易日判断、日期范围生成、时区转换）
5. 提供其他通用工具（文件操作、数据格式转换等）
...
├── visualizer.py              # Visualizer - 数据可视化
...
│          Visualizer                 │
│ ─────────────────────────────────── │
│ - _backend: str ('matplotlib'/'plotly')│
│ ─────────────────────────────────── │
│ + plot_candlestick(df, ...) -> Figure│
│ + plot_peaks_and_breakthroughs(...) │
│ + plot_indicators(df, ...) -> Figure│
│ + plot_equity_curve(...) -> Figure  │
│ + plot_drawdown(...) -> Figure      │
...
class Visualizer:
    ...
    def plot_candlestick(
        ...
        peaks: Optional[List[Tuple[str, float]]] = None,  # [(date, price), ...]
        breakthroughs: Optional[List[Tuple[str, float]]] = None,
        ...
        Args:
            df: 行情数据DataFrame（必须包含date, open, high, low, close, volume列）
            title: 图表标题
            peaks: 凸点列表 [(日期, 价格), ...]
            breakthroughs: 突破点列表
            figsize: 图表大小
    ...
    def plot_peaks_and_breakthroughs(
        self,
        df: pd.DataFrame,
        peaks: List[Dict],  # [{'date': ..., 'price': ..., 'quality_score': ...}, ...]
    ...
    def plot_indicators(
        self,
        df: pd.DataFrame,
        indicators: List[str],  # ['ma_20', 'rsi_14', 'volume']
    ...
    def plot_equity_curve(
        self,
        equity: pd.Series,  # 时间序列，权益曲线
        benchmark: Optional[pd.Series] = None,  # 基准曲线
    ...
    def save_figure(self, fig: plt.Figure, path: str, dpi: int = 100):
        """
        保存图表到文件
...
- **问题**：是否需要交互式图表（plotly）？
- **权衡**：plotly图表更漂亮、支持交互，但依赖较大
- **建议**：默认使用matplotlib，提供plotly作为可选后端
