...
7. **Per-Factor Gating**：BO 检测是局部事实（仅依赖当前 bar + active peaks），与因子 lookback 解耦；每个因子在自己的 `_calculate_xxx` 内自检 lookback 足够性，不足则返回 None。全局 `max_buffer` gate 已删除。
8. **三层范围元数据**：`preprocess_dataframe` 在返回的 df 上写 `df.attrs["range_meta"]`，承载 pkl / scan / compute / label buffer 的 ideal-actual 双值；下游 UI 通过 `UI/charts/range_utils.ChartRangeSpec` 消费。

### Per-Factor Gating 架构要点

- **Lookback SSOT**：`FeatureCalculator._effective_buffer(fi)` 定义每个因子的最小 idx 要求（SSOT）；新增因子必须在此注册，否则 strict 抛 `ValueError`。
- **对外接口**：`FeatureCalculator.max_effective_buffer(config)` 迭代完整 `FACTOR_REGISTRY`（含 inactive 因子）返回最大 lookback，供 `preprocess_dataframe` 和 `live.daily_runner._compute_download_days` 派生 buffer / 下载量。
- **None 语义**：`Breakout` dataclass 的 8 个字段（`volume / pbm / day_str / overshoot / pk_mom / pre_vol / ma_pos / annual_volatility`）为 `Optional[float]`，None 表示"数据不够算"。
- **FactorDetail.unavailable**：`BreakoutScorer` 产出的 `FactorDetail` 新增 `unavailable: bool` 字段，区分"数据不够"（灰色 N/A 渲染）和"算出来但未触发"（灰色）。下游 UI tooltip 据此做三态渲染。

### 三层范围元数据要点

`scanner` 的两个入口协作写 `df.attrs["range_meta"]`：
- `preprocess_dataframe`：在裁切后写入 pkl 起止 / scan ideal / compute ideal-actual / label_buffer ideal-actual / scan actual（初值由 start_date/end_date clamp 派生）。
- `compute_breakouts_from_dataframe`：从 `valid_start_index / valid_end_index` 派生 `scan_start_actual / scan_end_actual` 覆盖初值；actual 偏离 ideal 时 INFO 日志记录降级原因（pkl 起点晚 / pkl 终点早）。

**Why 两次写入**：JSON-cache 路径只跑 `preprocess_dataframe`（不跑 detector），仍需 spec 可构造；scanner 路径再精化 actual 以反映 valid window。
