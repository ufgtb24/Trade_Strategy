"""回归测试(final whole-branch review I1):scan_load_flat 必须为老 scan 文件补齐 debug_enabled_classes。

types.ts 里 debug_enabled_classes 是 non-optional 字段;pre-v4 scan 文件(实测
...
event_styles、漏 patch debug_enabled_classes,前端未来消费时会 undefined.forEach() 报
runtime TypeError。

用真实 run_scan_multi 落盘一份 scan 文件(与 test_scan_multi_pattern.py / test_scans_route_flat.py
同构造方式),再手动剔除 debug_enabled_classes 字段模拟 pre-v4 文件,过 GET /scans/{ts} 验证
...
def app_with_pre_v4_scan(tmp_path):
    ...
    debug_enabled_classes 字段模拟 pre-v4 scan 文件,最后建 TestClient。"""
    data = tmp_path / "data"
    data.mkdir()
    n = 200
    base = pd.DataFrame({
    ...
    module_paths = {"bottom_burst": "path2_apps.bottom_breakout_burst",
    ...
    # 模拟 pre-v4 scan 文件:剔除刚落盘文件里的 debug_enabled_classes(当前 serialize_pattern
    ...
    for pid in ("bottom_burst", "bo_only"):
        del blob["per_pattern"][pid]["pattern_spec"]["debug_enabled_classes"]
        assert "debug_enabled_classes" not in blob["per_pattern"][pid]["pattern_spec"]
...
def test_scan_load_backfills_debug_enabled_classes_for_hooked_pattern(app_with_pre_v4_scan):
    ...
    assert "debug_enabled_classes" in pattern_spec, (
        "scan_load_flat 必须为 pre-v4 scan 文件补齐 debug_enabled_classes 字段"
    )
    assert pattern_spec["debug_enabled_classes"] == ["tb"], (
        f"bottom_burst 应派生 ['tb'],实际 {pattern_spec['debug_enabled_classes']!r}"
    )


def test_scan_load_backfills_debug_enabled_classes_is_fresh_not_hardcoded(app_with_pre_v4_scan):
    ...
    assert "debug_enabled_classes" in pattern_spec
    assert pattern_spec["debug_enabled_classes"] == []
