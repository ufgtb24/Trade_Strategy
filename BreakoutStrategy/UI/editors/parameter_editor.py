...
class ParameterEditorWindow:
    ...
    def __init__(
        ...
        # 初始化注释解析器（从 scan_params.yaml 读取中文注释）
        scan_params_path = (
        ...
        self.comment_parser = YamlCommentParser(scan_params_path)
        ...
        # 默认加载 scan_params.yaml
        self._load_file_internal(scan_params_path)
    ...
    def _build_json_param_map(self) -> Dict[str, Any]:
        ...
        处理两种情况:
        1. 直接映射: breakout_detector.total_window -> detector_params.total_window
        ...
        param_map["breakout_detector.total_window"] = detector.get("total_window")
        param_map["breakout_detector.min_side_bars"] = detector.get("min_side_bars")
        param_map["breakout_detector.min_relative_height"] = detector.get("min_relative_height")
        param_map["breakout_detector.exceed_threshold"] = detector.get("exceed_threshold")
        param_map["breakout_detector.peak_supersede_threshold"] = detector.get("peak_supersede_threshold")
        ...
        # breakout_weights (JSON: bo_weight_* -> UI: breakout_weights.*)
        param_map["quality_scorer.breakout_weights.change"] = scorer.get("bo_weight_change")
        param_map["quality_scorer.breakout_weights.gap"] = scorer.get("bo_weight_gap")
        param_map["quality_scorer.breakout_weights.volume"] = scorer.get("bo_weight_volume")
        param_map["quality_scorer.breakout_weights.continuity"] = scorer.get("bo_weight_continuity")
        param_map["quality_scorer.breakout_weights.stability"] = scorer.get("bo_weight_stability")
        param_map["quality_scorer.breakout_weights.resistance"] = scorer.get("bo_weight_resistance")
        param_map["quality_scorer.breakout_weights.historical"] = scorer.get("bo_weight_historical")
    ...
    def load_from_file(self, file_path: str = None):
        ...
        if not file_path:
            default_dir = (
                self.loader.get_project_root() / "configs" / "params"
            )
    ...
    def save_to_file(self):
        ...
        # 检查参数组合错误（阻断性）
        errors = self._check_param_combination_errors()
        if errors:
            error_text = "\n\n".join(errors)
            messagebox.showerror(
    ...
    def _check_param_combination_errors(self) -> list:
        """
        检查参数组合是否有效，返回错误列表（阻断性）

        某些参数组合会导致不合理的结果，直接禁止使用
        """
        errors = []

        # 获取 breakout_detector 参数
        if "breakout_detector" not in self.section_map:
            return errors

        detector_params = self.section_map["breakout_detector"].get_all_values()

        peak_measure = detector_params.get("peak_measure", "body_top")
        breakout_modes = detector_params.get("breakout_modes", ["body_top"])

        # 检查无效组合：close 用于峰值，high 用于突破
        # 这会导致 high 很容易超过 close，频繁触发突破，产生重叠显示
        if peak_measure == "close" and "high" in breakout_modes:
            errors.append(
                "peak_measure=close + breakout_modes=[high] 组合无效：\n"
                "峰值用 close（较低），突破用 high（较高），\n"
                "会导致频繁触发突破和显示重叠。\n\n"
                "请修改参数：建议 peak_measure 与 breakout_modes 使用相同选项。"
            )
