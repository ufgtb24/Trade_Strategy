...
### 4.2 突破质量评分（Breakthrough Quality Score）- Bonus 乘法模型
...
score_breakthrough()   ──委托──>  get_breakthrough_score_breakdown_bonus()
```
...
**FeatureCalculator 职责**：从 BreakoutInfo 计算完整的 Breakthrough 对象

**核心特征**：
```yaml
突破类型（breakthrough_type）:
  - yang: 阳线突破（收盘 > 开盘）
  - yin: 阴线突破（收盘 < 开盘）
...
动量特征:
  - recent_breakthrough_count: 近期突破次数（momentum_window 内）
...
if breakout_info:
    ...
    breakthrough = calculator.enrich_breakthrough(
...
breakdown = scorer.get_breakthrough_score_breakdown_bonus(breakthrough)
...
- 删除的权重配置：`breakthrough_weights`, `resistance_weights`, `historical_weights`
...
- 删除的旧分解方法：`get_breakthrough_score_breakdown()`, `_get_resistance_importance()`, `_get_resistance_breakdown()`, `_get_historical_breakdown()`, `_get_momentum_breakdown()` (FeatureScoreDetail版)
