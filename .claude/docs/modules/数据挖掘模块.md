...
### 可选情感验证 (run_sentiment)

**Why**: 技术面合格的模板命中样本，可通过消息面进一步筛选。`run_sentiment=True` 在 OOS 验证后执行 Step 6：提取 top-K 命中样本 → 按 (ticker, breakout_date) 去重 → 并发调用 `news_sentiment.api.analyze` → 四级阈值分类（pass/reject/strong_reject/insufficient_data） → 统计 cascade_lift → 嵌入 validation_report.md 的 Section 7。

`run_sentiment` 严格依赖 `run_validation`：validation 未运行时，情感验证自动跳过（打印提示）。情感验证参数通过 `sentiment_config` 字典传入，包含阈值、回溯天数、并发数等。原 `cascade/` 子包已消除，逻辑内联为 `_run_sentiment_filter` 等私有函数。

`include_breakout_day`（默认 True）控制新闻采集窗口是否包含突破日当天。纳入突破日新闻的理由：label 从 bo_date+1 起算，突破日信息不泄露 label；与因子系统一致（volume/daystr 等均使用突破日 K 线数据）；冷门股往往在突破当天才产生新闻覆盖。

### 测试集扫描精确匹配

**Why**: `_should_rescan` 检查已有 `scan_results_test.json` 的日期范围与目标是否匹配。采用精确匹配（`!=`）而非覆盖判断，确保缩小日期范围时也会触发重新扫描。

### Per-Template Comparison 全量输出

**Why**: 验证报告 Section 3 输出所有模板（按 train median 降序），top-K 模板名后标 `*`。便于观察非 top-K 模板的测试表现，发现训练集排名靠后但测试集表现优异的模板。

### NaN 感知流水线（per-factor gating 的下游适配）

**Why**: per-factor gating 之后，因子可能因 lookback 不足返回 None；若沿用旧的 `.fillna(0)` 策略，会把"数据不够算"与"算出来是 0"混为一谈，导致：
- 反向因子（`lte`）误触发：`0 <= threshold` 在 threshold > 0 时总是 True，把本应跳过的样本当成触发
- 挖掘统计污染：raw 值分布中混入大量人造 0，Spearman 方向、分位数阈值、TPE 范围都偏斜

**实现**：
- `data_pipeline.prepare_raw_values` 去掉 `.fillna(0)`，保留 None/NaN 原样
- `data_pipeline.apply_binary_levels` 显式构造 `valid = raw.notna()` mask，NaN 样本不触发任一方向
- `threshold_optimizer.build_triggered_matrix` 用 `~np.isnan(raw)` 保护；TPE bounds 计算（`np.quantile`）先过滤 NaN；greedy beam 的候选百分位也过滤 NaN；全 NaN 因子 `continue` 跳过 trial
- `factor_diagnosis.write_diagnosed_yaml` 接 `audit_info` 参数，每因子写入 `valid_count / valid_ratio / buffer` 审计字段，便于排查"哪些因子在哪个样本组覆盖率低"

这套适配保证：per-factor gating 下的挖掘结果既反映真实阈值（不被伪 0 污染），又在 factor_diag.yaml 留下可观测的样本完整性快照。
...
| `template_validator.py` | Trial 物化 + 五维度样本外验证 + 可选情感筛选 | Yes |
...
| Step 4 | `archive_dir/` 全部中间产出 | `trials/<id>/filter.yaml` + `all_factor.yaml` + `validation_report.md`（含可选 Sentiment Filter 段落） |
