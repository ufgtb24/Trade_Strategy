...
class UIParamLoader:
    ...
    def __init__(self, params_path: Optional[str] = None):
        ...
        if params_path is None:
            # 默认路径：项目根目录/configs/params/scan_params.yaml
            params_path = self._project_root / "configs" / "params" / "scan_params.yaml"
        else:
            params_path = Path(params_path)

        self._params_path = params_path

        # 如果 scan_params.yaml 不存在，抛出错误
        if not self._params_path.exists():
            raise FileNotFoundError(
                f"参数文件不存在: {self._params_path}\n"
                "请确保 configs/params/scan_params.yaml 文件存在"
            )
    ...
    def get_detector_params(self) -> Dict[str, Any]:
        ...
        Returns:
            参数字典，包含: total_window, min_side_bars, min_relative_height,
                         ...
                         peak_measure, breakout_modes, use_cache, cache_dir
        """
        ...
        # breakout_modes 验证
        breakout_modes = detector_params.get('breakout_modes', ['body_top'])
        if not isinstance(breakout_modes, list):
            breakout_modes = [breakout_modes] if breakout_modes else ['body_top']
        # 过滤无效模式
        valid_modes = ['high', 'close', 'body_top']
        breakout_modes = [m for m in breakout_modes if m in valid_modes]
        if not breakout_modes:
            breakout_modes = ['body_top']
        ...
        validated = {
            ...
            'streak_window': self._validate_int(
                detector_params.get('streak_window', 20), 1, 9999, 20
            ),
        }
    ...
    def get_feature_calculator_params(self) -> Dict[str, Any]:
        ...
        Returns:
            参数字典，包含: stability_lookforward, continuity_lookback, spike_window
        ...
        validated = {
            'stability_lookforward': self._validate_int(
                feature_params.get('stability_lookforward', 10), 1, 9999, 10
            ),
            'continuity_lookback': self._validate_int(
                feature_params.get('continuity_lookback', 5), 1, 9999, 5
            ),
            'atr_period': self._validate_int(
                feature_params.get('atr_period', 14), 1, 9999, 14
            ),
            'gain_window': self._validate_int(
                feature_params.get('gain_window', 5), 1, 30, 5
            ),
            'pk_lookback': self._validate_int(
                feature_params.get('pk_lookback', 30), 1, 9999, 30
            ),
            'ma_period': self._validate_int(
                feature_params.get('ma_period', 200), 5, 500, 200
            ),
        }

        # ATR 配置传递到 FeatureCalculator（向后兼容旧配置）
    ...
    def get_scorer_params(self) -> Dict[str, Any]:
        ...
        validated['bonus_base_score'] = self._validate_int(
            quality_params.get('bonus_base_score', 50), 1, 9999, 50
        )
        ...
        # PBM bonus（突破前涨势强度）
        pbm_bonus = quality_params.get('pbm_bonus', {})
        validated['pbm_bonus'] = {
            'enabled': pbm_bonus.get('enabled', True),
            'thresholds': pbm_bonus.get('thresholds', [0.001, 0.003]),
            'values': pbm_bonus.get('values', [1.15, 1.30]),
        }

        # Streak bonus（连续突破）
        streak_bonus = quality_params.get('streak_bonus', {})
        validated['streak_bonus'] = {
            'enabled': streak_bonus.get('enabled', True),
            'thresholds': streak_bonus.get('thresholds', [2]),
            'values': streak_bonus.get('values', [1.20]),
        }
        ...
        # Overshoot penalty（超涨惩罚，基于 5 日波动率标准化，单位 σ）
        overshoot = quality_params.get('overshoot_penalty', {})
        validated['overshoot_penalty'] = {
            'enabled': overshoot.get('enabled', True),
            'thresholds': overshoot.get('thresholds', [3.0, 4.0]),
            'values': overshoot.get('values', [0.7, 0.4]),
        }

        # Intraday Return Vol bonus（日内涨幅波动率标准化）
        idr_vol_bonus = quality_params.get('intraday_return_vol_bonus', {})
        validated['intraday_return_vol_bonus'] = {
            'enabled': idr_vol_bonus.get('enabled', True),
            'thresholds': idr_vol_bonus.get('thresholds', [2.0, 3.0]),
            'values': idr_vol_bonus.get('values', [1.10, 1.20]),
        }

        # Gap Vol bonus（跳空波动率标准化）
        gap_vol_bonus = quality_params.get('gap_vol_bonus', {})
        validated['gap_vol_bonus'] = {
            'enabled': gap_vol_bonus.get('enabled', True),
            'thresholds': gap_vol_bonus.get('thresholds', [1.0, 2.0]),
            'values': gap_vol_bonus.get('values', [1.10, 1.15]),
        }

        # PK Momentum bonus（近期 peak 凹陷深度）
        pk_momentum_bonus = quality_params.get('pk_momentum_bonus', {})
