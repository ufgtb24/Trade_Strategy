...
describe('DetailSidebar (5-node: pattern roles + stream-source)', () => {
  ...
  it('命中匹配列表渲染,有 forward_return 时显示 ret_N 行', async () => {
    ...
    const scanWithLabel = {
      ...
      results: [
        {
          ...INLINE_SCAN.results[0],
          analysis: {
            ...INLINE_SCAN.results[0].analysis,
            matches: [
              { ...INLINE_SCAN.results[0].analysis.matches[0], forward_return: 0.05 },
            ],
          },
        },
      ],
