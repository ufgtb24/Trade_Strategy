...
def main() -> None:
    ...
    # ★ 控制方裁定(task-8-report.md 记录):tb.max_day_drop_pct 是 tb detector 内部的 emit
    # 门(filter_params 声明),不是 NodeSpec.where 阈值——探针分类=F,放 WHERE_LEVELS 会被
    # classify() 的纯 where 校验拒绝(ValueError)。改道放 SCAN_GRID(F 维不进检测组合,
    # detection_combos 会把它踢出笛卡尔积,效果等价于"作为一个只读维度出现在长表列里")。
