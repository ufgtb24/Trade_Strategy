...
| 入口 A / B / D / E | 漏检 4 入口固定索引 —— A=时段 brush 查失败 attempts（`scope=time`）/ B=拓扑点边查 miss_reasons 分布（`scope=nodes`）/ D=shift+click 两个 event 查为啥没连（`scope=pair`）/ E=CLI workflow `scripts/scan_top_miss.py` 批量 markdown 排序 |
...
| 候选对比表 / node table             | 某 node 漏斗行展开后就地显示的详情表：每 event 一行、每 where clause 一列（单元格 = `实测 (op 阈值) ✓/✗`）；行按 tier 着色；与 K 线 marker 双向选中联动 |
