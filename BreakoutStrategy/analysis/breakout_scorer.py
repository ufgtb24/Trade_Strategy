...
突破评分公式：
    总分 = BASE × age_bonus × test_bonus × height_bonus × peak_volume_bonus ×
           volume_bonus × breakout_day_strength_bonus × pbm_bonus ×
           streak_bonus × drought_bonus × pk_momentum_bonus × overshoot_penalty

模式标签（后处理）：
    基于主导 bonus 自动分类：momentum, high_resistance, volume_surge,
    trend_continuation, dormant_breakout, deep_rebound,
    power_breakout, basic
...
class BreakoutScorer:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.height_bonus_thresholds = height_bonus.get('thresholds', [0.20, 0.40, 0.70])
        self.height_bonus_values = height_bonus.get('values', [1.30, 1.60, 2.00])
        ...
        # PBM bonus（突破前涨势强度 Pre-Breakout Momentum，sigma 标准化）
        pbm_bonus = config.get('pbm_bonus', {})
        self.pbm_bonus_enabled = pbm_bonus.get('enabled', True)
        self.pbm_bonus_thresholds = pbm_bonus.get('thresholds', [0.70, 1.45])
        ...
        # Drought bonus（久旱逢甘霖，streak 的天然镜像）
        # 长时间无突破后突然产生突破，沉寂后的觉醒
        drought_bonus = config.get('drought_bonus', {})
        self.drought_bonus_enabled = drought_bonus.get('enabled', True)
        self.drought_bonus_thresholds = drought_bonus.get('thresholds', [60, 120])
        self.drought_bonus_values = drought_bonus.get('values', [1.15, 1.30])
        ...
        self.overshoot_penalty_thresholds = overshoot.get('thresholds', [4.0, 5.0])
    ...
    def _get_pbm_bonus(self, momentum: float) -> BonusDetail:
        ...
        momentum 已在 FeatureCalculator 源头标准化为 σ_N 单位：
        pbm_normalized = raw_pbm × √N / daily_vol

        Args:
            momentum: 已标准化的涨势强度值 (σ_N 单位)
    ...
    def _get_drought_bonus(self, days_since_last: Optional[int]) -> BonusDetail:
        """
        计算久旱逢甘霖 bonus（streak 的镜像）

        长时间无突破后突然产生突破，沉寂后的觉醒往往代表蓄势释放。
        首次突破（days_since_last=None）不触发。

        Args:
            days_since_last: 距上次突破的交易日间隔，None=首次突破

        Returns:
            BonusDetail
        """
        if not self.drought_bonus_enabled or days_since_last is None:
            return BonusDetail(
                name="Drought",
                raw_value=days_since_last if days_since_last is not None else 0,
                unit="d",
                bonus=1.0,
                triggered=False,
                level=0
        ...
        bonus, level = self._get_bonus_value(
            days_since_last,
            self.drought_bonus_thresholds,
            self.drought_bonus_values
        )

        return BonusDetail(
            name="Drought",
            raw_value=days_since_last,
            unit="d",
            bonus=bonus,
            triggered=(bonus > 1.0),
            level=level
    ...
    def _classify_pattern(self, bonuses: List[BonusDetail]) -> str:
        ...
        用 Height 替代 Age/Tests 作为分类维度（Age/Tests 统计无效，已禁用）

        模式定义（按优先级）：
        - 混合模式：power_breakout (放量+高阻力), deep_rebound (势能+高阻力)
        - 单一模式：momentum, high_resistance, volume_surge, trend_continuation
        - 兜底：dormant_breakout, basic
        ...
        height_level = b["Height"].level if "Height" in b else 0
        streak_level = b["Streak"].level if "Streak" in b else 0
        drought_level = b["Drought"].level if "Drought" in b else 0

        # 混合模式（多因子共振）
        if vol_level >= 1 and height_level >= 2:
            return "power_breakout"         # 放量 + 高阻力
        if pk_mom_level >= 1 and height_level >= 2:
            return "deep_rebound"           # 势能 + 高阻力

        # 单一模式
        if pk_mom_level >= 1:
            return "momentum"
        if height_level >= 2:
            return "high_resistance"        # 纯高阻力
        if vol_level >= 1:
            return "volume_surge"
        if streak_level >= 1:
    ...
    def get_breakout_score_breakdown_bonus(
        ...
        # Breakout Day Strength bonus（突破日强度，合并 IDR-Vol 和 Gap-Vol）
        ...
        drought_bonus = self._get_drought_bonus(breakout.days_since_last_breakout)
        bonuses.append(drought_bonus)
