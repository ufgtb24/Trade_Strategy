# -*- coding: utf-8 -*-
"""multivar_scan.py 冒烟脚本(Task 8 Step 2):小网格 + 小股票子集,验证进程池/断点续跑/台账
三件事真的工作。由 .claude/skills/tune-gates/multivar_scan.py 复制而来,只改 main() 常量
(TICKER_REGEX/OUT_DIR/SCAN_GRID/WHERE_LEVELS/WORKERS/SHARD_STOCKS),其余逻辑逐字不变。
产物写 smoke/ 子目录,不提交(见同目录 .gitignore)。

Task 10 修复轮 3 后:主脚本的 sys.path 已改成显式 `REPO / ".claude/skills/tune-gates"`(不再
依赖 `Path(__file__).parent`),与本冒烟副本完全同源,不再需要单独的 sys.path 偏离说明。

★★ 维护提醒:主脚本 `.claude/skills/tune-gates/multivar_scan.py` 改动后,本文件必须同步
重新复制(除 main() 里那几个冒烟规模常量外,逐字保持一致)——否则冒烟验证的就不是主脚本
的真实行为(Task 8 修复轮 1 review 命中过一次此类漂移)。
"""
...
# 复制后 Path(__file__).parent 会指向复制目标目录而非 tune-gates/,导致 multivar_core 导入失败
# (与 region_find.py 同款写法)。
sys.path.insert(0, str(REPO)); sys.path.insert(0, str(REPO / ".claude/skills/tune-gates"))

from multivar_core import (ScanConfig, classify, col_of, detection_combos, row_columns,  # noqa: E402
                           scan_one_stock)
...
def _worker(pkl_path, cfg: ScanConfig, buf_start, buf_end, start_date, end_date, volume_min):
    ...
    try:
        ...
        # 窗口 20 与 multivar_core.scan_one_stock 内部那次独立计算耦合(Task 8 修复轮 1
...
def main() -> None:
    ...
    WORKERS = 4
    # ^(A[A-C].*|NA)$(修复轮 2·1,修复轮 3 订正因果):在原 ^A[A-C] 前缀宇宙基础上精确
    # 并入 "NA" 一支——datasets/pkls/NA.pkl 真实存在,原冒烟宇宙不含它、
    # keep_default_na=False 这个 kwarg 被删掉不会被任何现有验收发现。★ 并入 NA 本身
    # **不构成**回归锁(修复轮 3 实证纠偏,原注释"一旦回归第二轮会变成 1 待扫"是错的):
    # bb_v1 上 "NA" 恰好有 match,而 done 集第一来源是 parquet(带类型的列式格式,字符串
    # 原样返回,根本不经过 pandas 的 NA 字符集)——删掉两处 read_csv 的 kwarg 后,"NA" 仍
    # 会先经 parquet 路径正常进 done,第二轮照样"0 待扫",这条正则改动**判别不出**回归
    # (已用 sed 删 kwarg 实测坐实)。并入 NA 只是让下面 done 集的 NaN 断言有机会被真实
    # 触碰到("NA" 出现在候选宇宙里,才有 CSV 路径可能把它读错的场景);真正的锁是那条
    # `assert not any(pd.isna(x) for x in done)`。".*" 后缀保证 A[A-C] 支仍是前缀匹配
    # (与原正则等价,不缩小既有 108 只宇宙),"$" 只对 "NA" 支生效、不会误并入
    # NAAS/NABL 等 23 只同前缀股票。
    TICKER_REGEX = r"^(A[A-C].*|NA)$"
    ...
    SCAN_GRID = {("burst", "gap_max"):          [8, 12],
                 ("tb", "stop_confirm_bars"):   [1, 2]}
    WHERE_LEVELS = {("burst", "first_drought_min"): [0, 20]}
    OUT_DIR = "docs/research/2026-08-25_multivar-bb_v1/smoke/"
    SHARD_STOCKS = 30
    ...
    n_combo = len(detection_combos(SCAN_GRID, cls))
