...
- **A-KOF-ID**(共享 #seq 字节一致):`edges=[A→B(0,0)]`,k=1,`A=[ev(5,5),ev(5,5)]`,`B=[ev(5,5),ev(5,5)]` → ids ⊆ {"kof_5_5","kof_5_5#1"},`run()` 不抛;`end_idx` 升序;`set(flatten(role_index.values()))==set(children)`。
