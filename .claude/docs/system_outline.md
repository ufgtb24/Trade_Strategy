...
`analysis` 产出突破和评分，`mining` 挖掘阈值并生成模板回写 configs，其中 `template_validator` 的 OOS 验证流程可选集成情感筛选（直接调用 `news_sentiment` 公共 API）。`UI` 作为策略开发入口驱动全流程；`live` 作为生产盯盘入口消费定稿 Trial，两者共享 `analysis` 扫描器和 `UI/charts/` 图表组件但数据目录与状态机独立。
...
| 交互式 UI | `BreakoutStrategy/UI/` | [modules/交互式UI.md](modules/交互式UI.md) | 批量扫描、参数编辑、模板过滤、图表浏览（开发态） |
