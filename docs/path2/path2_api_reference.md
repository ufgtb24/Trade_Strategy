...
> **版本**:v0.1(2026-05-15)
> **对齐**:`docs/research/path2_spec.md` v0.1
...
| **Pattern** | 形容词(描述事件之间的约束) | **6 个关系算子** + **1 个组合子** + `TemporalEdge` 声明类型 |
...
| 关系算子 | `Before(anchor, predicate=None, *, window, stream=None)` | anchor 之前 N bar 内满足;predicate 省略=窗内 stream 存在性 |
| 关系算子 | `At(anchor, predicate)` | anchor 自身满足 |
| 关系算子 | `After(anchor, predicate=None, *, window, stream=None)` | anchor 之后 N bar 内满足;predicate 省略=窗内 stream 存在性 |
...
- **标准 PatternDetector**:`Chain`/`Dag`/`Kof`/`Neg` + `PatternMatch`,消费 `TemporalEdge` 声明——见 `docs/research/path2_spec.md` §7.1 与算法权威 `docs/research/path2_algo_core_redesign.md`。
...
驱动 Detector 的推荐入口。包裹 `detector.detect(*source)`,在产出流上施加**跨事件安全网**:`end_idx` 升序、`event_id` 单 run 唯一、yield 出的对象必须是 `Event`。违规在源头即抛错(见 §1.2.2 / `docs/research/path2_spec.md` §5.1),而非让脏数据流到下游。
...
### 2.1 `Before(anchor, predicate=None, *, window, stream=None)`
...
| **② bar 索引** | `(idx: int) -> bool`(`stream=None` 形态) | 窗口内**任一 bar** 求值 True |
| **③ 事件流** | `(event: Event) -> bool` + `stream` | `stream` 中**任一事件**满足 `anchor.start_idx - window <= event.end_idx < anchor.start_idx` 且 `predicate(event)` |

- `predicate=None` 且 `stream=None` → 调用时 `ValueError`(无流可作存在性检测,协议"绝不静默退化")
- `window` 必填 keyword-only(须 `window=N` 具名传)

#### 用法示例

```python
# 形态 ②(bar 索引):BO 前 20 bar 内某天收盘 > MA20
Before(bo, predicate=lambda i: df.close[i] > df.ma20[i], window=20)

# 形态 ③(事件流):BO 前 20 bar 内 vol_stream 里有 ratio >= 2 的事件
...
| predicate=None 且 stream=None 期望放行 | 协议:无流可作存在性检测 → ValueError(window<=0 时 False 短路优先) |
| 形态 ③ 忘传 `stream` | predicate 是事件签名时**必须**传 stream |
...
与 `Before` 对称:`predicate` 省略(`predicate=None`)+ `stream` = 窗内 `stream` 存在性,判据留在产 `stream` 的 Detector;`predicate=None` 且 `stream=None` → 调用时 `ValueError`;`window` 必填 keyword-only;`window<=0` 返回 False 且短路优先于 ValueError。
...
### 2.6 `Overlaps` — 区间关系存在性(宽义 overlap=任意相交)
...
|---|---|---|---|---|
| `Before(a, p=None, *, w, stream)` | a 之前 | w bar | bar 索引 / 事件(p 省略=窗内 stream 存在性) | "BO 之前 20 天稳定" |
| `At(a, p)` | a 自身 | 1(自身) | 事件 | "BO 自身 drought≥30" |
| `After(a, p=None, *, w, stream)` | a 之后 | w bar | bar 索引 / 事件(p 省略=窗内 stream 存在性) | "BO 之后 5 天 vol spike"(跨流) |
| `Over(es, attr, r, op, thr)` | 容器 | — | — | "簇内累计破峰≥5" |
| `Any(es, p)` | 容器 | — | 事件 | "簇内任一 BO 放量" |
| `Overlaps(a, mode, p=None, *, stream)` | a 与 stream 事件区间相交(5 mode) | — | 事件(p 省略=仅判区间关系) | `not Overlaps(m, "contains", stream=d)` = "m 期间不发生 d" |
...
| §2.6 Overlaps | spec §2.6 |
| §3.1 Pattern.all | spec §2.7 |
