...
def test_integrated_system():
    ...
    if multi_peak_breakthroughs:
        avg_peaks = sum(bt.num_peaks_broken for bt in multi_peak_breakthroughs) / len(multi_peak_breakthroughs)
    ...
    sorted_breakthroughs = sorted(breakthroughs, key=lambda x: x.quality_score or 0, reverse=True)

    for i, bt in enumerate(sorted_breakthroughs[:5], 1):
        print(f'[{i}] {bt.date} (索引: {bt.index})')
        print(f'    突破价格: ${bt.price:.2f}')
        print(f'    突破类型: {bt.breakthrough_type}, 涨幅: {bt.price_change_pct*100:.2f}%')
        print(f'    放量: {bt.volume_surge_ratio:.2f}倍, 连续性: {bt.continuity_days}天, 稳定性: {bt.stability_score:.0f}%')
        print(f'    ---')
        ...
        if bt.num_peaks_broken > 1:
            ...
            if qualities:
                print(f'    峰值质量: min={min(qualities):.1f}, avg={sum(qualities)/len(qualities):.1f}, max={max(qualities):.1f}')
        else:
    ...
    if multi_peak_breakthroughs:
        ...
        for i, peak in enumerate(best_multi.broken_peaks, 1):
            ...
            print(f'    压制: 左{peak.left_suppression_days}天, 右{peak.right_suppression_days}天')
            print()
        ...
        print(f'  价格差距: ${best_multi.peak_price_range:.2f} ({best_multi.peak_price_range/avg_price*100:.2f}%)')
        print()
