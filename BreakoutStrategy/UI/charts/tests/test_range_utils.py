...
def test_scan_start_degraded_when_pkl_starts_later():
    spec = _make_spec(
        scan_start_ideal=date(2024, 1, 1),
        pkl_start=date(2024, 6, 1),
    )
    assert spec.scan_start_degraded is True


def test_scan_end_degraded_when_pkl_ends_earlier():
    spec = _make_spec(
        scan_end_ideal=date(2024, 12, 31),
        pkl_end=date(2024, 10, 15),
    )
    assert spec.scan_end_degraded is True


def test_compute_buffer_degraded_when_pkl_starts_later():
    spec = _make_spec(
        compute_start_ideal=date(2023, 11, 13),
        pkl_start=date(2024, 2, 1),
    )
    assert spec.compute_buffer_degraded is True


def test_no_false_degradation_when_ideal_falls_on_weekend():
    """ideal 落在周末 / 节假日时，actual 会抵近到相邻交易日；pkl 边界覆盖请求范围则不算降级。"""
    spec = _make_spec(
        scan_end_ideal=date(2025, 6, 1),       # Sunday
        scan_end_actual=date(2025, 5, 30),      # Friday
        compute_start_ideal=date(2023, 2, 11),  # Saturday
        compute_start_actual=date(2023, 2, 13), # Monday
        pkl_start=date(2021, 1, 1),
        pkl_end=date(2026, 4, 16),
    )
    assert spec.scan_end_degraded is False
    assert spec.compute_buffer_degraded is False
...
def test_collect_warnings_includes_scan_start():
    # pkl_start > scan_start_ideal 必然也 > compute_start_ideal，所以 scan_start
    # 和 MA buffer 会同时降级 —— 这里只验证 scan_start 警告存在并包含 actual 日期。
    spec = _make_spec(
        scan_start_ideal=date(2024, 1, 1),
    ...
    assert any("scan_start" in w and "2024-06-01" in w for w in warnings)
