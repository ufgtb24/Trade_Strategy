...
class MatchList(ttk.Frame):
    ...
    def _build_treeview(self) -> None:
        ...
        self.tree.bind("<Down>",  lambda _e: self._handle_key_navigate(1, self._get_current_selected()))
        self.tree.bind("<Up>",    lambda _e: self._handle_key_navigate(-1, self._get_current_selected()))
    ...
    def _get_current_selected(self) -> "MatchedBreakout | None":
        """MatchList 自身不持有 current（SSOT 在 LiveApp），通过 row_current tag
        反查：哪一行挂了 row_current，哪一行就是当前选中。"""
        for iid in self.tree.get_children():
    ...
    def navigate(self, direction: int) -> None:
        ...
        供外部（如 chart 快捷键）在 tree 未获焦的情况下仍能驱动列表导航。
        底层复用 _handle_key_navigate —— 无选中时跳第 0 行；已到边界不 wrap。
        """
        self._handle_key_navigate(direction, self._get_current_selected())
