...
class MarkerComponent:
    ...
    def draw_breakthroughs(
        ...
        colors = colors or {}
        marker_color = colors.get("breakthrough_marker", "#0000FF")
        text_bg_color = colors.get("breakthrough_text_bg", "#FFFFFF")
        text_score_color = colors.get("breakthrough_text_score", "#FF0000")
        ...
        for bt in breakthroughs:
            ...
            # 获取基准高度
            if has_high and 0 <= bt_x < len(df):
                base_price = df.iloc[bt_x][high_col]
            else:
                base_price = bt.price
            ...
            # 检查是否与峰值重叠
            is_overlap = bt_x in peak_indices

            # 确定文本位置
            if is_overlap:
                # 如果重叠，显示在峰值信息上方
                # KBar -> Marker(0.5) -> PeakText(1.1) -> BTText(2.0)
                # 之前 PeakText 是 marker_y(0.5) + 0.6 = 1.1
                # 所以这里设为 2.0 比较安全 (4% Range)
                text_y = base_price + offset_unit * 2.2
            else:
                # 如果不重叠，显示在 KBar 上方
                # KBar -> BTText(0.8) (1.6% Range)
                text_y = base_price + offset_unit * 0.6

            # 绘制突破分数
            if (
                show_score
                and hasattr(bt, "quality_score")
                and bt.quality_score is not None
            ):
    ...
    def draw_resistance_zones(
        ...
        for bt in breakthroughs:
            ...
            width = bt_x - earliest_peak_index
