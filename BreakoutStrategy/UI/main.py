...
from .managers import NavigationManager, ScanManager, compute_breakouts_from_dataframe
...
class InteractiveUI:
    ...
    def _on_stock_selected(self, symbol: str, stock_data: dict):
        ...
        if self._can_use_json_cache(symbol, params, df):
            ...
            breakouts, detector = self._load_from_json_cache(symbol, params, df)
            load_end = time.time()
            print(
            ...
            #     )
            #     breakouts, detector = self._full_computation(symbol, params, df)
        else:
            # 完整计算（慢速路径）
            load_start = time.time()
            breakouts, detector = self._full_computation(symbol, params, df)
        ...
        self.chart_manager.update_chart(
            df, breakouts, detector, symbol, display_options
        )
    ...
    def _load_from_json_cache(
        ...
        for bo_data in stock_data.get("breakouts", []):
            ...
            # 重新映射索引：根据日期在新 DataFrame 中查找位置
            try:
                new_index = df.index.get_loc(pd.Timestamp(bo_date))
                # get_loc() 可能返回整数、切片或布尔数组，需要处理
                if isinstance(new_index, slice):
                    # 如果是切片，取第一个索引
                    new_index = new_index.start
            ...
            # 处理可能为 None 的字段
            price_change_pct = bo_data.get("price_change_pct")
            gap_up_pct = bo_data.get("gap_up_pct")
            volume_surge_ratio = bo_data.get("volume_surge_ratio")
