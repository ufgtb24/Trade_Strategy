...
from scripts.data.preprocess import StockPreprocessor
...
def worker(task_queue, save_root, days_from_now, append_data, file_format):
    while not task_queue.empty():
        tic = task_queue.get()
        save_path = os.path.join(save_root, tic + ('.csv' if file_format == 'csv' else '.pkl'))
        try:
...
def multi_download_stock(tickers, save_root, days_from_now, append_data, clear,num_workers=os.cpu_count(), file_format='pkl'):
    if clear:
        # 删除 save_root 下的所有文件
...
if __name__ == '__main__':
    ...
    if os.path.exists('datasets/stock_list.pkl') and use_cache:
        ...
        tickers = ak.get_us_stock_name()
        all_tickers = tickers.symbol.tolist()

    # Start downloading stock data for all tickers
    start_time = datetime.datetime.now()
    multi_download_stock(all_tickers,
