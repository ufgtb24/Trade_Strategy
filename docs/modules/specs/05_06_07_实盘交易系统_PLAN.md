...
**模块路径**：`BreakoutStrategy/monitoring/`, `trading/`, `risk/`
**创建日期**：2025-11-16
...
class SignalDetector:
    ...
    def detect_realtime_signal(
        ...
        Args:
            current_price: 当前价格
            breakout_info: 突破信息字典

        Returns:
            是否触发买入信号
        """
        # 获取突破日K线实体上沿
        # 假设breakout_info包含breakout_price（突破日最高价）
        # 实际应该包含open, close，取max作为实体上沿
        breakout_price = breakout_info.get('price')

        # 简化版：使用突破价格
        trigger_price = breakout_price * (1 + self.realtime_threshold_pct)
...
```python
from BreakoutStrategy.monitoring import RealtimeMonitor, MonitorScheduler
from BreakoutStrategy.trading import OrderManager, PositionTracker
from BreakoutStrategy.risk import StopLossManager, TakeProfitManager, PositionSizer
