...
"""W.attr 工厂携带结构化元数据 + measure;调用接口 (x)->bool 不变。"""
...
def test_attr_still_returns_bool():
    ...
    assert fn(_E(drought=88)) is True
    assert fn(_E(drought=10)) is False


def test_attr_carries_meta_and_measure():
    fn = W.attr("drought", ">=", 60)
    assert fn.meta == {"kind": "attr", "field": "drought", "op": ">=", "threshold": 60}
    assert fn.measure(_E(drought=88)) == 88


def test_none_value_still_false():
    fn = W.attr("drought", ">=", 60)
    assert fn(_E(drought=None)) is False   # 保留旧 None 短路语义
    assert fn.measure(_E(drought=None)) is None
