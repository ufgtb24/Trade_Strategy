...
## 1. Project Overview
This project implements a **Breakout Stock Selection Strategy** for US markets. It identifies historical resistance levels (peaks), detects breakouts, and evaluates them using a multi-stage scoring system. The system includes an analysis engine, an interactive UI, and a configuration management system.

## 2. Architecture & Core Components

### Core Logic (`BreakoutStrategy/analysis/`)
- **Peak Detection**: Identifies local maxima in price history (`breakout_detector.py`).
- **Breakout Detection**: Detects when price exceeds historical peaks (`breakout_detector.py`).
...
- **Structure**:
    ...
    - `configs/analysis/params/`: Strategy parameter sets (e.g., `breakout_0.yaml`).
- **Loaders**: `BreakoutStrategy/config_loader.py` handles loading and validation.

## 3. Critical Workflows

### Running the System
- **Batch Scan**: Use the UI's "New Scan" button to scan stocks and generate JSON results in `outputs/scan_results/`. Configure scan parameters via `configs/scan_config.yaml`.
...
- **Breakout/Breakout (bo)**: Price exceeding a Peak.
- **Suppression**: The duration a price has been held below a peak.

### Best Practices
- **Core Principles**: Adhere to First Principles and Occam's Razor. Emphasize exploring the essence of requirements. Avoid over-engineering.
- **Paths**: Always use `pathlib.Path` for file operations. Avoid string manipulation for paths.
- **Imports**: Use absolute imports (e.g., `from BreakoutStrategy.analysis import ...`) to avoid circular dependency issues.
