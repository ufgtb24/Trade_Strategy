...
class StockListPanel:
    ...
    def __init__(
        ...
        # 联合信号高亮阈值
        self._high_ws_threshold = self._load_highlight_threshold()

        # 创建UI
        self._create_ui()
        self._load_column_config()

    def _load_highlight_threshold(self) -> float:
        """从 ui_config.yaml 读取 high_ws_threshold"""
        from ..config import get_ui_config_loader

        config_loader = get_ui_config_loader()
        highlight = config_loader.get_all_config().get("ui", {}).get("composite_highlight", {})
        return highlight.get("high_ws_threshold", 5.0)
    ...
    def _create_ui(self):
        ...
        # 垂直滚动条（高对比度样式：浅色滑块 + 深色槽道）
        self.v_scrollbar = tk.Scrollbar(
            list_frame,
            orient=tk.VERTICAL,
            command=self._on_vsb_scroll,
            bg="#ffffff",              # 滑块颜色（浅灰）
            troughcolor="#2d2d2d",     # 槽道颜色（深灰）
            activebackground="#c8ff00", # 悬停时颜色（更亮）
        ...
        # 水平滚动条（高对比度样式：浅色滑块 + 深色槽道）
        self.h_scrollbar = tk.Scrollbar(
            main_frame,
            orient=tk.HORIZONTAL,
            bg="#ffffff",              # 滑块颜色（浅灰）
            troughcolor="#2d2d2d",     # 槽道颜色（深灰）
            activebackground="#ddff00", # 悬停时颜色（更亮）
            highlightthickness=0,
            bd=0,
            width=14,
        )
        ...
        # 绑定全局上下键导航（使用 bind_all 确保焦点无关）
        root = self.parent.winfo_toplevel()
        root.bind_all("<Up>", self._on_global_key_navigate)
        root.bind_all("<Down>", self._on_global_key_navigate)
    ...
    def _is_editable_widget(self, widget) -> bool:
        """判断是否为可编辑控件（Entry/Spinbox/Text等）"""
        try:
            cls = widget.winfo_class()
        except Exception:
            return False
        return cls in {"Entry", "TEntry", "Spinbox", "TSpinbox",
                       "Combobox", "TCombobox", "Text"}

    def _on_global_key_navigate(self, event):
        """处理全局上下键导航"""
        # 若焦点在可编辑控件上，允许默认行为（如移动光标）
        try:
            root = self.parent.winfo_toplevel()
            focus_w = root.focus_get()
            if focus_w is not None and self._is_editable_widget(focus_w):
                return None  # 不拦截，让事件继续传播
        except Exception:
            pass
        ...
        current = self.get_selected_symbol()
        if not current:
            # 无选中项，选择第一个
            if self.filtered_data:
                self._select_and_notify(self.filtered_data[0]["symbol"])
            return "break"

        # 找到当前索引（跳过临时行）
        symbols = [s["symbol"] for s in self.filtered_data]
        try:
        ...
        if event.keysym == "Up":
            ...
            new_idx = min(len(symbols) - 1, idx + 1)

        if new_idx != idx:
            self._select_and_notify(symbols[new_idx])

        return "break"  # 阻止默认行为

    def _select_and_notify(self, symbol: str):
        """选中指定股票并触发回调"""
        self._restore_selection(symbol)
        # _restore_selection 阻止了事件回调，手动调用 _handle_selection 触发
        self._handle_selection(symbol)
    ...
    def load_data(self, scan_results: Dict):
        ...
        Args:
            scan_results: 扫描结果字典，包含 results 列表
        """
        # 清除临时行状态（重新加载数据时）
        ...
        for result in scan_results.get("results", []):
            ...
            item["b_count"] = result.get("b_count", result.get("bo_count", 0))
            item["v_count"] = result.get("v_count", result.get("hv_count", 0))
            item["y_count"] = result.get("y_count", result.get("by_count", 0))
            item["d_count"] = result.get("d_count", result.get("dt_count", 0))

            # 前瞻涨幅（可选字段）
            item["forward_return"] = result.get("forward_return")

            # 保存信号列表供 K 线图使用
            item["_signals"] = result.get("signals", [])

            self.stock_data.append(item)

        # 配置列
        if self.stock_data:
            columns = ["signal_count", "weighted_sum", "sequence_label",
                        "b_count", "v_count", "y_count", "d_count"]
            self._configure_tree_columns(columns)

            # 检查是否有 forward_return 数据（至少一个非 None 值），如果没有则从 visible_columns 中移除
            has_forward_return = any(
                item.get("forward_return") is not None for item in self.stock_data
            )
            if not has_forward_return:
                # 从配置的 visible_columns 中过滤掉 forward_return
                from ..config import get_ui_config_loader
                config_loader = get_ui_config_loader()
                config = config_loader.get_stock_list_column_config()
                visible = config.get("visible_columns", [])
                if "forward_return" in visible:
                    visible = [c for c in visible if c != "forward_return"]
                    self._configure_tree_columns(visible)
    ...
    def _update_tree(self, restore_selection: bool = True):
        ...
        for stock in self.filtered_data:
            ...
            for col in columns:
                val = stock.get(col, "")
                # 格式化浮点数
                if col in ("amplitude", "forward_return"):
                    if val is None:
                        values.append("-")
                    elif isinstance(val, (int, float)):
                        values.append(f"{val * 100:.0f}%")
                    else:
    ...
    def sort_by(self, column: str, reverse: bool = None):
        ...
        # 排序后自动选择第一行
        if self.filtered_data:
            first_symbol = self.filtered_data[0]["symbol"]
            # 重置上次选中记录，确保回调被触发
            self._last_selected_symbol = None
            # 设置选中状态（会自动触发 _on_fixed_select -> _handle_selection）
            self.fixed_tree.selection_set(first_symbol)
            self.fixed_tree.see(first_symbol)
