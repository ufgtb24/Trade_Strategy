...
from path2.eval import match_forward_returns, match_first_passage   # noqa: E402  (FR + FP 四态)
...
# ═══ 任务参数(按研究改) ═══
SCAN = REPO / "outputs/path2_web/scans/scan-wide-bb_v1-202401-202601.json"   # 宽进底座 scan(scan-wide.py 产出)
...
ATR_WINDOW = 14               # params tb.atr_window(控制列用)
# 宽进快照已含 max_day_drop_pct=null(scan-wide.py override),无需 TB_OVERRIDES。
TB_OVERRIDES = {}
# 简单 --scan= --out= override(scan-oos 外推对比用:对每个 oos scan 提取宽表)
for _a in [a for a in sys.argv[1:] if a.startswith("--scan=")]:
    SCAN = Path(_a.split("=", 1)[1])
for _a in [a for a in sys.argv[1:] if a.startswith("--out=")]:
    OUT_CSV = Path(_a.split("=", 1)[1])
...
def compute_features(win: pd.DataFrame, o: dict) -> dict:
    ...
    纪律:
      ...
      - 除法要守护分母(<=0 → np.nan)。
    """
    from path2.atoms.throwback_v1 import _revert_max_day_drop   # 与 detector 同源口径
    # 毒药闸原始量(第 2 步事后切档第 4 道闸):回踩段最大单日跌幅,
    # 口径 = _revert_max_day_drop(df, bo_idx, confirm_idx)——detector 判定完全同款,
    # 宽进关闸后此列随行,事后切「删掉 day_drop ≥ X」与真扫等价。
    return {"day_drop": _revert_max_day_drop(win, o["bo_idx"], o["tb_start"])}
...
def main() -> pd.DataFrame:
    ...
    fp_k = meta.get("first_passage_k", 5.0)   # 与 scan 的 first_passage_k 同口径(plateau 的 fp 列)
    ...
    for sym, ids in scan_ids.items():
        ...
        for m in kept:
            ...
            o = dict(symbol=sym, tb_id=tb.instance_id, bo_id=tb.anchor_bo_id,
                     ...
                     # 筛子层闸原始量(事后切档零成本;burst where 可引用字段,见 breakout.py:78-92)
                     burst_first_drought=getattr(burst, "first_drought", None),
                     burst_distinct_pk=getattr(burst, "distinct_pk", None),
                     burst_max_bar_vol_ratio=getattr(burst, "max_bar_vol_ratio", None),
                     burst_peak_age_max=getattr(burst, "peak_age_max", None),
                     atr=a)
            ...
            if lab is None or ref is None or abs(lab - ref) >= 1e-12:
                ...
                continue
            o["label"] = float(ref)
            # FP 四态计数(plateau 的 fp 列素材):切档时按档聚合 up/(up+down+both)
            fp = match_first_passage(m, "tb", win, horizon,
                                     k=fp_k, sample_window=(lo, hi))
            o.update(fp_up=fp["up"], fp_down=fp["down"],
