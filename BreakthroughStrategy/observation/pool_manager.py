...
if TYPE_CHECKING:
    from BreakthroughStrategy.analysis import Breakthrough
...
class PoolManager:
    ...
    使用示例（回测）：
        ...
        # 添加突破
        pool_mgr.add_from_breakthrough(breakthrough)
    ...
    def add_from_breakthrough(self, breakthrough: 'Breakthrough') -> bool:
        ...
        """
        # 质量评分过滤
        if breakthrough.quality_score is not None:
            if breakthrough.quality_score < self.min_quality_threshold:
                print(f"[PoolManager] {breakthrough.symbol} quality score "
                      f"{breakthrough.quality_score:.2f} < {self.min_quality_threshold}, skipping")
                return False
    ...
    def add_from_search_results(self,
                                ...
                                breakthroughs: Optional[Dict[str, 'Breakthrough']] = None) -> int:
        """
        从搜索结果批量添加

        Args:
            results_df: 搜索结果 DataFrame，必须包含 'symbol' 和 'breakthrough_date' 列
            breakthroughs: 可选的 Breakthrough 对象字典，key 为 symbol
        ...
        for _, row in results_df.iterrows():
            ...
            # 获取突破日期
            bt_date = row.get('breakthrough_date', row.get('bt_date'))
            if isinstance(bt_date, str):
                bt_date = date.fromisoformat(bt_date)
            elif isinstance(bt_date, pd.Timestamp):
    ...
    def re_add_after_trade(self, symbol: str, entry_info: Dict) -> bool:
        ...
        Args:
            symbol: 股票代码
            entry_info: 条目信息字典，包含 breakthrough_date 等字段

        Returns:
            是否添加成功
        """
        bt_date = entry_info.get('breakthrough_date')
        if isinstance(bt_date, str):
            bt_date = date.fromisoformat(bt_date)
    ...
    def _evaluate_buy_condition(self,
        ...
        if reference_price <= 0:
            reference_price = entry.breakthrough_price
    ...
    def _create_entry_from_row(self, row: pd.Series) -> PoolEntry:
        ...
        return PoolEntry(
            ...
            add_date=self.time_provider.get_current_date(),
            breakthrough_date=bt_date,
            quality_score=row.get('breakthrough_quality_score',
                                  row.get('quality_score', 0)),
            breakthrough_price=row.get('breakthrough_price',
                                       row.get('bt_price', 0)),
