...
### 2.1 BreakoutDetector 初始化

**文件**：`BreakoutStrategy/analysis/breakout_detector.py`
...
for threshold in [0.02, 0.03, 0.04, 0.05]:
    detector = BreakoutDetector(..., shadow_threshold=threshold)
...
def compare_shadow_filter_effect(symbol: str, df: pd.DataFrame):
    ...
    # 原始方案
    detector_orig = BreakoutDetector(
        symbol,
    ...
    # 影线过滤方案
    detector_filter = BreakoutDetector(
        symbol,
        apply_shadow_filter=True,
        shadow_threshold=0.03
    )
...
```python
# 方案A：通过参数禁用
detector = BreakoutDetector(..., apply_shadow_filter=False)

# 方案B：通过阈值禁用（影线永不过长）
detector = BreakoutDetector(..., shadow_threshold=999.0)
```
