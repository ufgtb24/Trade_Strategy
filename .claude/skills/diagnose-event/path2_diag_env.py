...
输入 symbol + instance_id(+可选 scan 路径),自动从 scan 推断诊断环境:
...
def resolve_target_pattern(pattern_ids: list[str], instance_id: str) -> str:
    """诊断目标 pattern = pattern_ids 中非 bo_only 的那个(xxx,真正诊断的)。
    bo_only 是 xxx 的漏检参照系,非诊断目标。
    instance_id 的 node_id 前缀辅助:tb_*/burst_* 必属 xxx;bo_* 共有,默认归 xxx。
    ...
    if len(candidates) == 1:
        return candidates[0]
    # 多个 xxx:instance_id 前缀无法可靠消歧(bo_* 共有;tb/burst 需查 pattern 拓扑),报错让用户指定
    raise ValueError(f"多个非 bo_only pattern {candidates},instance_id={instance_id} 无法消歧,请指定目标 pattern")
...
def params_summary(params) -> str:
    ...
    tb 字段随 ThrowbackDetectorV4 换代(2026-08-16):六字段 max_rise_k/
    stop_confirm_bars/vol_window/anchor_mode/max_span/measure;旧 v3 的
    big_rise_k/max_start_gap 已不存在,引用即 AttributeError。"""
    ...
    return (f"burst(min_bos={b.min_bos}/first_drought_min={b.first_drought_min}/"
            ...
            f"tb(max_rise_k={t.max_rise_k}/stop_confirm_bars={t.stop_confirm_bars}/vol_window={t.vol_window}/"
            f"anchor_mode={t.anchor_mode}/max_span={t.max_span}/measure={t.measure}) "
...
def format_declaration(env: dict) -> str:
    ...
    return "\n".join([
        ...
        f"诊断目标: {env['symbol']} 的 {env['instance_id']}",
        "========================================",
    ])
...
def diagnose_env(symbol: str, instance_id: str, scan_path: str | None = None) -> dict:
    ...
    pid = resolve_target_pattern(scan["pattern_ids"], instance_id)
    ...
    return {
        "symbol": symbol, "instance_id": instance_id,
...
if __name__ == "__main__":
    ...
    SYMBOL = "DVLT"
    INSTANCE_ID = "tb_257_258"
    SCAN_PATH: str | None = None   # None=最新 scan;或给绝对路径

    env = diagnose_env(SYMBOL, INSTANCE_ID, SCAN_PATH)
    print(format_declaration(env))
