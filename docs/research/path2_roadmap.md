...
权威定义见 `docs/research/path2_spec.md`;教学见 `docs/path2/path2_tutorial.md` / `docs/path2/path2_api_reference.md`;尖锐问答见 `path2_qa.md`。
...
**Ad-hoc 协议层改进:`Before`/`After` predicate 可选化 —— 已实现、已验证、已合入 `complex_framing`(@ cb7a570,FF)**:

- 非路线 #N(真实使用暴露的泄漏抽象 ad-hoc 修正,同 role_index 收敛先例)。`path2/operators.py` 的 `Before`/`After`:`predicate` 必填 → `Optional[Callable]=None`(=窗内 `stream` 存在性,"什么算合格事件"归产 `stream` 的 Detector,不在算子处二次施判);`window` 改 keyword-only 必填;`predicate=None`+`stream=None` → 调用点 `ValueError`;`window<=0 → False` 短路优先于该 ValueError。仅 `Before`/`After`,`At`/`Over`/`Any` 零改动;硬切换无 shim;现有 `test_operators` 用例未改即零破坏证据;179 pytest 全过(基线 169 + 新增 10)
- 流程:brainstorm(逐问直答,签名形态/作用域/退化语义)→spec→plan→subagent-driven(2 任务,各两阶段 review + 折 minor / 补残留矛盾)→ holistic READY-TO-MERGE → FF 合入 → worktree/branch 已清理
- 文档:design `docs/superpowers/specs/2026-05-19-path2-operator-optional-predicate-design.md`;plan `docs/superpowers/plans/2026-05-19-path2-operator-optional-predicate.md`;spec §2.1/§2.3 + api_reference + tutorial 已写回
- 过程关键事件:plan 漏 scope api_reference 自带 §2.1/§2.3 详细块 + tutorial cheatsheet `After` 行残留旧签名 → spec review 捕获自相矛盾,同实现 agent 补齐至"一致写回"
- post-merge 收尾已做:`update-ai-context` 已刷新 `.claude/docs/modules/path2.md`(协议层算子决策加 predicate 可选化);`system_outline.md` 无需改(path2 非 BreakoutStrategy 模块,同 #3 理由)

**Ad-hoc 协议层改进:`Overlaps` 多 mode 区间关系算子 —— 已实现、已验证、已合入 `complex_framing`(@ 595354d,FF)**:

- 非路线 #N(brainstorm 起点为补全 Before/During/After 三元的单语义 `During`,经用户扩展为多 mode 区间关系算子)。`path2/operators.py` 新增 `Overlaps(anchor, mode, predicate=None, *, stream)->bool`:5 mode(`contains`/`within`/`overlapped_front`/`overlapped_back`/`equals`)宽义相交存在性,mode 可传一组任一命中;包含用 `≤`、重叠要真交叠(meets 不命中)、A==B 三命中、点事件 contains/within/equals 等价;`stream` 必填无索引形态、无 window;未知/空 mode→ValueError;涵盖原 `During` 用例(`not Overlaps(m,"contains",stream=d)`)。仅新增,`Before`/`At`/`After`/`Over`/`Any` 零改动;现有 `test_operators` 用例未改即零破坏;192 pytest 全过(基线 179 + 新增 13)
- 流程:brainstorm(逐问,部分派 tom——tom 裁单语义 During 最小充分,被用户有意覆盖为多 mode,留痕于设计稿 §1.2 + tom agent-memory)→spec→plan→subagent-driven(2 任务,各两阶段 review + 折 minor + 修文档计数/交叉引用回归)→ holistic READY-TO-MERGE → FF 合入 → worktree/branch 已清理
- 文档:design `docs/superpowers/specs/2026-05-22-path2-overlaps-operator-design.md`;plan `docs/superpowers/plans/2026-05-23-path2-overlaps-operator.md`;spec §2.6 + api_reference + tutorial 已写回;**先导研究** `docs/research/path2_batch_relops_analysis.md`(events×stream 多对多批量化裁"不做"——During/Overlaps 仍 1-对-多,外层遍历调用方自管)
- post-merge 收尾已做:`update-ai-context` 已刷新 `.claude/docs/modules/path2.md`(协议层算子加 `Overlaps`);`system_outline.md` 无需改(同前理由)
