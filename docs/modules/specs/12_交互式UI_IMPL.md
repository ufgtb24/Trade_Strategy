...
UI 模块是一个基于 Tkinter 的桌面交互式应用，作为整个系统的**统一入口**，承担批量扫描、股票列表浏览、K线图表可视化、参数实时调整等完整功能。
...
**解决方案**：
- `param_editor_schema.py`：从 `FACTOR_REGISTRY` 动态生成编辑器 Schema，新因子自动出现在编辑面板
- `input_factory.py`：`BaseParameterInput` 在 `param_value=None` 时 fallback 到 `param_config.get("default")`，确保各类控件（Int/Float/Bool/List）正常渲染
...
UI 顶部参数面板提供 **Add to Pool** 按钮，点击后将当前显示的突破添加到观察池。
...
观察池管理器使用懒加载模式，仅在首次使用时初始化，避免不必要的资源占用：
...
UI 使用 `BreakoutJSONAdapter` 从 JSON 缓存重建 Breakout 对象，然后添加到观察池：

```mermaid
flowchart LR
    JSON[JSON 扫描结果] --> Adapter[BreakoutJSONAdapter]
    Adapter --> Breakout[Breakout 对象]
    Breakout --> PoolMgr[PoolManager]
    PoolMgr --> Signal[买入信号评估]
```
