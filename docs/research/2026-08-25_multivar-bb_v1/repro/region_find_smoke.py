# -*- coding: utf-8 -*-
"""region_find.py 冒烟脚本(Task 10 Step 4):在 Task 8 冒烟长表上验证 bootstrap/校正/出图/报告
真的能跑通。由 .claude/skills/tune-gates/region_find.py 复制而来,只改 main() 常量
(LONGTABLE_DIR/OUT_DIR/COMBO_LEVELS/FILTER_PREDS/WHERE_PREDS/REF_POINT/MIN_COUNT_PER_FOLD/
B_BOOT/FLAG_RULES),其余逻辑逐字不变。产物写 smoke/ 子目录,不提交(见同目录 .gitignore)。

主脚本的 sys.path 已改成显式 `REPO / ".claude/skills/tune-gates"`(不再依赖
`Path(__file__).parent`),与本冒烟副本完全同源,不再需要单独的 sys.path 偏离说明。

★ 维护提醒:主脚本 `.claude/skills/tune-gates/region_find.py` 改动后,本文件必须同步
重新复制(除 main() 里那几个冒烟规模常量外,逐字保持一致)——否则冒烟验证的就不是
主脚本的真实行为。
"""
...
def main() -> None:
    LONGTABLE_DIR = "docs/research/2026-08-25_multivar-bb_v1/smoke/longtable/"
    HEAD_BUFFER = 250
    # 轴序 = COMBO_LEVELS(真扫维,含 F 维——F 维在下面按 FILTER_PREDS 变谓词,不出现在这里)+ 后面的 preds
    # 冒烟:仅 2 真扫维,无 F 维
    COMBO_LEVELS = {"burst.gap_max": [8, 12], "tb.stop_confirm_bars": [1, 2]}
    FILTER_PREDS = []
    WHERE_PREDS = [("burst.first_drought", ">=", [0, 20])]
    FOLD_COL, FOLDS = "fold_Y", ["2024", "2025"]
    MIN_COUNT_PER_FOLD = 5
    REF_POINT = {"burst.gap_max": 8, "tb.stop_confirm_bars": 2}     # 参照 = 此检测组合 × 全部谓词最松档
    NEIGHBOR_AXES = "all"
    B_BOOT, SEED, TOP_N = 30, 0, 20
    FLAG_RULES = []
