...
class StockListPanel:
    ...
    def _create_ui(self):
        ...
        self._label_type_var = tk.StringVar(value="max")
        ...
        # 信息栏（选中行号/总数 + 突破总数），数据加载后显示
        self._info_bar = ttk.Frame(container)
        self._info_index_label = ttk.Label(self._info_bar, text="")
        self._info_index_label.pack(side=tk.LEFT, padx=(4, 10))
        self._info_breakouts_label = ttk.Label(self._info_bar, text="")
        self._info_breakouts_label.pack(side=tk.LEFT)

        # 列表容器 (包含两个Treeview和滚动条)
        self._list_frame = ttk.Frame(container)
    ...
    def _update_info_bar(self):
        ...
        selected = self.get_selected_symbol()
        index = "-"
        if selected:
            for i, s in enumerate(self.filtered_data):
                if s["symbol"] == selected:
                    index = i + 1
                    break

        total_bos = sum(s.get("total_breakouts", 0) for s in self.filtered_data)

        self._info_index_label.config(text=f"{index}/{total}")
        self._info_breakouts_label.config(text=f"Breakouts: {total_bos}")
    ...
    def load_data(self, scan_results: Dict):
        ...
        # 显示信息栏（首次加载时 pack，位于列表上方）
        if not self._info_bar.winfo_manager():
            self._info_bar.pack(fill=tk.X, padx=2, pady=(2, 0),
                                before=self._list_frame)
        self._update_info_bar()
    ...
    def _calculate_label_stats(self, breakouts: list) -> dict:
        ...
        # 转为百分比形式（0.048 → 4.8）
        valid_labels = [v * 100 for v in valid_labels]
        ...
        if stats["best_quality"] is not None:
            stats["best_quality"] *= 100
    ...
    def add_t_count(self, matched_dict: dict):
        """添加 t_count 列并填充数据。

        Args:
            matched_dict: {symbol: [matched_breakout_indices]}
        """
        # 注入 t_count 到 stock_data
        for stock in self.stock_data:
            symbol = stock["symbol"]
            matched = matched_dict.get(symbol, [])
            stock["t_count"] = len(matched)
        ...
        # 确保 t_count 列在可见列中
        columns = list(self.main_tree["columns"])
        if "t_count" not in columns:
            columns.append("t_count")
            self._configure_tree_columns(columns)

        self._update_tree()

    def remove_t_count(self):
        """移除 t_count 列和数据。"""
        # 从 stock_data 中移除 t_count
        for stock in self.stock_data:
            stock.pop("t_count", None)
        ...
        # 从可见列中移除 t_count
        columns = list(self.main_tree["columns"])
        if "t_count" in columns:
            columns.remove("t_count")
            self._configure_tree_columns(columns)

        self._update_tree()

    def update_t_count(self, matched_dict: dict):
        """更新已有的 t_count 值并刷新。
