...
# (3) 关系算子(都返回 bool)
Before(anchor_event, predicate=None, window=N, stream=...)  # anchor 之前 N bar 内满足;predicate 省略=窗内 stream 存在性
At    (anchor_event, predicate)                       # anchor 当下满足
After (anchor_event, predicate=None, window=N, stream=...) # anchor 之后 N bar 内满足;predicate 省略=窗内 stream 存在性
...
> **硬约束**:Event 子类必须 `@dataclass(frozen=True)`,**不是裸 `@dataclass`**。`Event` 基类是 frozen,Python 在**类定义期**就会拒绝非 frozen 子类继承 frozen 基类(直接 `TypeError`)。事件一旦 yield 即不可变,才能安全跨层复用——这是 §0.3"Row 落地=字段完成"的配套保证。下文所有示例代码均按此写。
...
```
╔═══════════ stdlib 层(已就绪 · 协议层之上 · 不绑实现) ═══════════╗
║  Detector 模板        id 便利            标准 PatternDetector       ║
║  (BarwiseDetector)    (span_id)          (Chain/Dag/Kof/Neg         ║
║                                           + PatternMatch            ║
║                                           · LEF-DFS 实现,见 §10.5) ║
║  ※ stdlib 不含任何 Event 子类(领域字段使用方私有,不可预沉淀)    ║
╚═══════════╤═══════════════════╤══════════════════════╤════════════════╝
            │ ⑧提供现成件       │ ⑧                    │ ⑧
════════════┼═══════════════════┼══════════════════════┼═══ 协议层(瘦·稳定)
            ▼                   ▼                      ▼
...
| ⑧ | stdlib → 协议层 | stdlib 把 Detector 模板(`BarwiseDetector`)/ id 便利(`span_id`)/ PatternDetector(`Chain/Dag/Kof/Neg`)**沉淀成标准件**(协议层之上,不绑实现)。**不含任何 Event 子类**——领域字段使用方私有,不可预沉淀 |
...
`After(anchor, predicate=None, *, window, stream=None)` = "anchor 之后 window bar 内,stream 中至少一个事件满足 predicate;predicate 省略则只判定窗内 stream 存在任一事件,window 须 keyword 传"。
...
> 安全网受运行时开关门控;生产环境可整体关掉走零开销直通。详见 `docs/research/path2_spec.md` §1.2.5 / §5.1。
...
> 详见 `docs/research/path2_qa.md` Q1 备忘 B、`docs/research/path2_spec.md` §7.1、算法权威 `docs/research/path2_algo_core_redesign.md`。
