...
class ScanConfigDialog:
    ...
    def _init_variables(self):
        ...
        self.label_config_var = tk.StringVar(value="20")  # 回测标签配置
    ...
    def _create_ui(self):
        ...
        # 分隔线
        ttk.Separator(self.global_frame, orient="horizontal").pack(
            fill=tk.X, pady=8
        )

        # 价格范围
        price_row = ttk.Frame(self.global_frame)
        price_row.pack(fill=tk.X, pady=2)
        ttk.Label(price_row, text="Min BO Price:", width=14).pack(side=tk.LEFT)
        ttk.Entry(price_row, textvariable=self.min_price_var, width=10).pack(
            side=tk.LEFT
        )
        ttk.Label(price_row, text="  Max BO Price:", width=14).pack(side=tk.LEFT)
        ttk.Entry(price_row, textvariable=self.max_price_var, width=10).pack(
            side=tk.LEFT
        )

        # 最小成交量
        vol_row = ttk.Frame(self.global_frame)
        vol_row.pack(fill=tk.X, pady=2)
        ttk.Label(vol_row, text="Min Volume:", width=12).pack(side=tk.LEFT)
        ttk.Entry(vol_row, textvariable=self.min_volume_var, width=15).pack(
            side=tk.LEFT
        )
        ttk.Label(vol_row, text="(empty = no limit)").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(label_row, text="(max days, e.g. 40)").pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _load_current_config(self):
        ...
        # Global 模式 - 股票筛选条件
        min_price, max_price, min_volume = loader.get_filter_config()
        self.min_price_var.set(str(min_price) if min_price is not None else "")
        self.max_price_var.set(str(max_price) if max_price is not None else "")
        self.min_volume_var.set(str(min_volume) if min_volume is not None else "")
    ...
    def _get_config_from_ui(self) -> dict:
        ...
        min_volume_str = self.min_volume_var.get().strip()
        config["min_price"] = float(min_price_str) if min_price_str else None
        config["max_price"] = float(max_price_str) if max_price_str else None
        config["min_volume"] = int(min_volume_str) if min_volume_str else None
    ...
    def _save_config(self):
        ...
        try:
            ...
            loader.set_filter_config(
                config["min_price"], config["max_price"], config["min_volume"]
            )
