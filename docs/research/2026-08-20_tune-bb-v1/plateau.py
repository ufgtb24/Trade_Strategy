...
def main():
    # ── 参数(按需改这里) ──
    CSV = "docs/research/2026-08-20_tune-bb-v1/gates.csv"    # 输入宽表(cut_gates.py 产出)
    OUT_DIR = "docs/research/2026-08-20_tune-bb-v1"          # 输出目录(json/md/png)
    REL_TOL = 0.05                     # relative 模式容差(无 se 列时生效)
    MIN_MATCH = 100                    # match 功效线
    # 简单 --csv= --out= override(250 底座重做用)
    import sys
    for _a in [a for a in sys.argv[1:] if a.startswith("--csv=")]:
        CSV = _a.split("=", 1)[1]
    for _a in [a for a in sys.argv[1:] if a.startswith("--out=")]:
        OUT_DIR = _a.split("=", 1)[1]
    # ──────────────────
    out = Path(OUT_DIR)
