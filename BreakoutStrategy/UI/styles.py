...
SIGNAL_COLORS = {
    "B": "#0059FF",  # 蓝色 - 突破 (Breakout)
    "V": "#D6C530",  # 金色 - 超大成交量 (High Volume)
    "Y": "#FF0000",  # 红色 - 大阳线 (Big Yang)
    "D": "#9C27B0",  # 紫色 - 双底 (Double Trough)
}
...
CHART_COLORS = {
    ...
    # 被取代峰值标记（superseded by new peak）
    "peak_marker_superseded": "#808080",    # 被取代峰值标记（灰色）
    "peak_text_id_superseded": "#000000",   # 被取代峰值ID文字（黑色）
    # 突破标记
    "breakout_marker": "#0000FF",      # 突破标记（蓝色）
    "breakout_text_bg": "#FFFFFF",     # 突破文字背景（白色）
    "breakout_text_score": "#FF0000",  # 突破分数文字（红色）
    # 阻力带和支撑带
    "resistance_zone": "#FFA500",     # 阻力带（橙色）
    "support_zone": "#3C82C3",        # 支撑带（浅蓝色）
    "support_zone_broken": "#FF0000", # 已失效支撑带（灰色）
    # 十字线
    "crosshair_normal": "#0088CC",    # 普通模式十字线（蓝色）
    "crosshair_ctrl": "#FF6600",      # Ctrl模式横线（橙色）
    # Label 缓冲区
    "label_buffer_divider": "#888888",  # 分割线（灰色）
    "label_buffer_bg": "#F5F5F5",       # 背景（浅灰）
    # 均线颜色
    "ma_50": "#FFA500",    # MA50（橙色）
    "ma_150": "#9370DB",   # MA150（紫色）
    "ma_200": "#4169E1",   # MA200（蓝色）
    # 低谷标记
    "trough_marker": "#9C27B0",  # Trough1 标记（紫色）
    # 支撑 Trough 标记（D 信号后的支撑测试点）
    "support_trough_marker": "#000000",      # 支撑 trough 标记（黑色）
    "support_trough_text_id": "#000000",     # 支撑 trough ID 文字（黑色）
}
