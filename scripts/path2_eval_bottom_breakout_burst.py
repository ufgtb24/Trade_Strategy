"""并行扫描 datasets/pkls/ 全集,评估 bottom_breakout_burst 买点(tb)的 N 日前瞻收益。

pattern 质量自动化检测:每只股双端缓冲切窗(首部供指标 warm-up、尾部供 label,
参考 BreakoutStrategy/analysis/scanner.py 的 preprocess_dataframe),缓冲窗上跑
analyze,取每个 match 的 END_ROLE(买点窗)算 close[t+N]/close[t]-1 的买点日均值,
输出 per-买点明细 CSV + 每 horizon 汇总(count/mean/median/win_rate)。

口径说明:缓冲窗上的 match 集合与 path2_filter_bottom_breakout_burst.py(严格
...
参数全部在 main() 起始声明(无 argparse,CLAUDE.md 入口规范)。
    ...
    uv run python scripts/path2_eval_bottom_breakout_burst.py
"""
...
def _eval_ticker(
    ...
    horizons: tuple[int, ...], end_role: str, head_buffer_trading_days: int,
    ...
    买点落尾部缓冲的属窗外信号、丢弃);去重 = 同 ticker 内按 end_role event_id
    (多 match 共享同一买点窗只计一次,评估对象是买点)。
    ...
    try:
        ...
        for m in res.matches:
            ...
            rets = match_forward_returns(m, end_role, win, horizons)
            row = {
                "ticker": ticker,
                "tb_start_date": str(buy_date)[:10],
                "tb_end_date": str(win["date"].iat[ev.end_idx])[:10],
...
def main() -> None:
    ...
    OUTPUT_CSV: Path = REPO / "outputs" / "path2_eval_bottom_breakout_burst.csv"
    SUMMARY_FILE: Path = REPO / "outputs" / "path2_eval_bottom_breakout_burst_summary.txt"
    START_DATE: str = "2024-01-01"             # 扫描区间(买点有效性按此判)
    END_DATE: str = "2025-01-01"
    HORIZONS: tuple[int, ...] = (5, 10, 20)    # N 日收益的 N 列表
    END_ROLE: str = "tb"                       # 买点 role
    ...
    print(
        ...
        f"window=[{START_DATE}, {END_DATE}] horizons={HORIZONS} end_role={END_ROLE} ..."
    )
    all_rows: list[dict] = []
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futures = [
            ex.submit(_eval_ticker, p, PARAMS, START_DATE, END_DATE,
                      HORIZONS, END_ROLE, HEAD_BUFFER_TRADING_DAYS)
            for p in pkls
        ]
    ...
    lines = [
        f"window=[{START_DATE}, {END_DATE}] end_role={END_ROLE} "
