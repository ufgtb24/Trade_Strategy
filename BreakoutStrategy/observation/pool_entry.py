...
class PoolEntry:
    ...
    核心概念：
        PoolEntry 不是突破本身，也不是K线数据，而是突破发生后创建的"跟踪记录"。
        它记录了：
        - 突破的历史信息（突破价、峰值价、质量评分）
        - 当前的跟踪状态（active/bought/failed 等）
        - 评估过程中的累积信息（评估次数、价格极值）

    生命周期：
        1. 检测到突破 -> 创建 PoolEntry，加入实时池
        2. 实时池中每日评估 -> 更新 last_evaluation_score
        3. 满足买入条件 -> 生成 BuySignal，标记为 bought
        4. 不满足条件 -> 转移到日K池继续观察，或标记为 failed 移出

    与其他概念的区别：
        - Breakout: 突破检测的结果对象，包含突破时刻的完整信息
        - PoolEntry: 从 Breakout 创建的跟踪记录，用于后续买入评估
        - bar/current_bar: 当前K线数据 (OHLCV)，用于评估时的价格输入
