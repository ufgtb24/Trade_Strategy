...
**Architecture:** 四 Detector = 一个核心(标签解析 → edges 拓扑构建 → earliest-feasible 约束推进)的四种约束形态。Chain = Dag 加严线性校验的退化特例,二者共用 Dag 推进核心;Kof = 滑窗计数 + 有界缓冲;Neg = 正向子图 + forbid 谓词过滤。协议层(`path2/`)零改动。
...
## Task 5: Dag earliest-feasible 推进核心

design §3.2 + §3.1。核心算法:拓扑序 + 区间剪枝 + earliest-feasible + 非重叠贪心;`gap = later.start_idx - earlier.end_idx`(协议 §1.3.1)。Chain 复用此核心(Task 6)。
...
design §3.1 + §4。Chain = Dag 推进 + 构造期 `validate_chain`。
...
design §3.2。多入度;构造期 `validate_dag`(环检测)。
...
design §3.3。n = edges 条数,k = 至少满足条数;earliest-feasible + 非重叠贪心;封口=锚 max_gap 视界过后,有界缓冲按 end_idx 排序弹出;同 (label,s,e) 撞 → `#<seq>`。
