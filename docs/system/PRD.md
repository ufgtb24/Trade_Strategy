...
| **分析层** | [突破扫描（analysis）](../modules/specs/02_突破扫描模块_IMPL.md) | 识别历史凸点（阻力位）、检测突破走势、对峰值和突破进行质量评分（策略核心算法）(已实现) |
| **应用层** | [搜索系统（search）](../modules/specs/03_搜索系统_PLAN.md) | 扫描美股市场，搜索历史突破股票，按质量评分过滤候选标的 |
| **应用层** | [观察池（observation）](../modules/specs/04_观察池系统_IMPL.md) | 管理实时池和日K池，负责股票入池、出池、池间转换，跟踪潜在突破标的 (已实现) |
| **应用层** | [Simple Pool（simple_pool）](../modules/specs/14_Simple_Pool_IMPL.md) | MVP 版观察池，即时判断模型，4 核心参数，替代状态机方案 (已实现) |
...
| **验证层** | [数据挖掘（mining）](../modules/specs/15_数据挖掘模块_IMPL.md) | 统一因子注册、统计分析、阈值优化（Beam Search + Optuna TPE）、生成挖掘参数文件 (已实现) |
| **辅助层** | [新闻情感分析（news_sentiment）](../modules/specs/16_新闻情感分析_IMPL.md) | 多源新闻采集、可插拔LLM情感分析、时间衰减加权、SQLite缓存，辅助突破可靠度判断 (已实现) |
...
| 突破点 | Breakout Point/Breakout/bo | 股价高于凸点最高价，确认突破 |
